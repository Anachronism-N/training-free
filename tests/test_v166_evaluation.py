...
import prepare_v166_vbench_comparison as comparison  # noqa: E402
import prepare_v166_minimal_review as review  # noqa: E402
import run_v166_vbench_long as vbench  # noqa: E402
...
def test_v166_summary_uses_exclusive_groups_and_no_aggregate_gate() -> None:
    rows = {
...
def test_v166_summary_rejects_nonduplicate_custom_prompt_score() -> None:
...
def test_v166_shells_use_new_grid_and_corrected_analysis() -> None:
    generation = (SCRIPTS / "run_v166_multiscale_motion_moviebench16.sh").read_text(
        encoding="utf-8"
    )
    evaluation = (SCRIPTS / "run_v166_vbench_long.sh").read_text(
        encoding="utf-8"
    )
    assert "reservoir2_multiscaledir1" in generation
    assert "reservoir2_multiscalemotion1" in generation
    assert "run_v166_vbench_long.py" in evaluation
    assert "analyze_v166_corrected_metrics.py" in evaluation
    assert "prepare_v166_minimal_review.py" in evaluation
