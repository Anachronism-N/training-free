#!/usr/bin/env python3
"""Create an automatic, deterministic, at-most-four-video v167 review."""
...
def blind_order_key(prompt: int, method: str) -> str:
    raw = f"v167-minimal-review|{prompt}|{method}".encode("utf-8")
...
def selected_prompts(analysis: dict) -> list[dict]:
    if analysis.get("experiment") != "v167_corrected_metric_analysis":
        raise ValueError("not a v167 corrected metric report")
...
def prepare(args: argparse.Namespace) -> dict:
    ...
    if len(expanded) > 4:
        raise ValueError("v167 minimal review exceeds four videos")
    key_rows = []
    reviewer_rows = []
    ...
    key_sha = write_frozen_json(
        ...
        {
            "version": 1,
            "experiment": "v167_minimal_blind_review",
