#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for the frozen v194 transfer grid."""
...
from prepare_v194_cf_checkpoint_transfer import METHODS, NUM_OUTPUT_FRAMES, PROMPT_COUNT
...
def analyze(payload: dict) -> dict:
    ...
    if tuple(rows) != METHODS or dimensions != DIMENSIONS or payload.get("missing"):
        raise ValueError("v194 VBench summary violates the frozen grid")
    ...
    return {
        ...
        "claim_boundary": (
            "Aggregate means are descriptive. The transfer decision requires paired "
            "prompt-level confidence intervals, temporal safety, and same-prompt "
            "effect comparison with the frozen Self-Forcing reference."
        ),
    }
...
def render(report: dict) -> str:
    lines = [
        "# v194 Causal Checkpoint Transfer VBench-Long Core-9",
        "",
...
def configure() -> dict:
    ...
    if (
        ...
        raise ValueError("invalid v194 VBench comparison contract")
    base.RUN_LABEL = "v194_cf_checkpoint_transfer64"
    base.SUMMARY_EXPERIMENT = EXPERIMENT
    base.ANALYSIS_STEM = "v194_vbench_analysis"
    base.SUMMARY_TITLE = "v194 Causal Checkpoint Transfer VBench-Long"
