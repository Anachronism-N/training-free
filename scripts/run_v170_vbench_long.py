#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for v170."""

from __future__ import annotations

import run_v154_vbench_long as base
from prepare_v170_vbench_comparison import (
...
def analyze(payload: dict) -> dict:
    ...
    if tuple(rows) != METHODS or dimensions != DIMENSIONS or payload.get("missing"):
        raise ValueError("v170 VBench summary violates the frozen grid")
    for method in METHODS:
        row = rows[method]
        if set(row) != set(DIMENSIONS):
            raise ValueError(f"{method}: incomplete v170 VBench dimensions")
        if (
    ...
    return {
        ...
        "claim_boundary": (
            "This aggregate is not a method ranking. Run the matched v170 "
            "analysis to separate policy effects from replica noise."
        ),
    }
...
def configure_base() -> None:
    ...
    base.ANALYSIS_STEM = "v170_vbench_analysis"
    base.SUMMARY_TITLE = "v170 Matched Attribution VBench-Long Core-9"
