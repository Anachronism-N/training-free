...
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/v155_profile_aligned_moviebench16/full7}"
NODE_RANK="${NODE_RANK:-0}"
NUM_NODES="${NUM_NODES:-4}"
GPU_LIST="${GPU_LIST:-0,1,2,3,4,5,6,7}"

DEFAULT_REUSE_ROOT="${ROOT}/runs/v154_history_critical_moviebench16/full8"
if [[ -z "${V155_REUSE_V154_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V155_REUSE_V154_ROOT="${DEFAULT_REUSE_ROOT}"
fi

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v155_profile_aligned_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
...
case "${ACTION}" in
  ...
  status)
    ...
    ;;
  blind)
    "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v155_blind_review.py" \
      --run-root "${OUT_ROOT}"
    ;;
  package)
    package_path="${OUT_ROOT}/v155_diagnostics.tar.gz"
