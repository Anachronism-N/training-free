#!/usr/bin/env python3
"""Create a deterministic, method-blind review package for v156."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--run-root",
        type=Path,
        default=root / "runs" / "v156_profile_exact_moviebench16" / "full7",
    )
    parser.add_argument(
        "--prompt-manifest",
        type=Path,
    ...
    if (
        ...
        or published.get("experiment") != "v156_profile_exact_moviebench16"
        ...
        raise SystemExit("v156 manifests violate the blind-review contract")
    ...
    base.write_frozen(
        output_root / "reviewer" / "v156_review_sheet.csv",
        buffer.getvalue().encode("utf-8"),
    )
    key = {
        "version": 1,
        "experiment": "v156_profile_exact_moviebench16_blind_review",
        "seed": args.seed,
    ...
    base.write_frozen(
        output_root / "private" / "v156_blind_key.json",
        base.canonical_json(key),
    )
    print(
        f"[v156-blind] PASS videos={len(key_rows)} links={link_modes} "
