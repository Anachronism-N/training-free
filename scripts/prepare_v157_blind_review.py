#!/usr/bin/env python3
"""Create a deterministic, method-blind review package for v157."""
...
from prepare_v157_vbench_comparison import METHODS, PROMPT_COUNT
...
def main() -> None:
    ...
    if (
        ...
        raise SystemExit("v157 manifests violate the blind-review contract")
    ...
    base.write_frozen(
        output_root / "reviewer" / "v157_review_sheet.csv",
    ...
    base.write_frozen(
        output_root / "private" / "v157_blind_key.json",
        base.canonical_json(key),
    )
    print(
        f"[v157-blind] PASS videos={len(key_rows)} links={link_modes} "
