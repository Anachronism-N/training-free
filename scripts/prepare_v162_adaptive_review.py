#!/usr/bin/env python3
"""Prepare one blinded 12-video adaptive review wave for v162."""
...
from analyze_v162_automated_screen import METHODS, REVIEW_METHODS
from prepare_v161_adaptive_review import SCORE_COLUMNS


ROOT = Path(__file__).resolve().parents[1]
PROMPT_COUNT = 16
RANDOM_SEED = 1622026
SOURCE_EXPERIMENT = "v162_freshmotion4_moviebench16"
    ...
    for prompt_index in prompt_indices:
        ...
        for slot, method in enumerate(methods):
            ...
            if not source.is_file():
                raise ValueError(f"missing v162 video: {source}")
            code = hashlib.sha256(
...
def main() -> None:
    ...
    if (
        ...
        raise ValueError("v162 adaptive-review manifests violate the contract")
    ...
    base.write_frozen(
        output_root
        / "reviewer"
        / f"v162_wave{args.wave}_review_sheet.csv",
        buffer.getvalue().encode("utf-8"),
    )
    key = {
        "version": 1,
        "experiment": "v162_adaptive_blind_review",
        "wave": args.wave,
        "seed": args.seed + args.wave,
        "methods": list(REVIEW_METHODS),
        "prompt_indices": prompt_indices,
    ...
    base.write_frozen(
        output_root
        / "private"
        / f"v162_wave{args.wave}_blind_key.json",
        base.canonical_json(key),
    )
    instructions = f"""# v162 Adaptive Blind Review: Wave {args.wave}
...
"""
    ...
    print(
        f"[v162-review] wave={args.wave} videos={len(key_rows)} "
