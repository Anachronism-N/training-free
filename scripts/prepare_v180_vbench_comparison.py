#!/usr/bin/env python3
"""Materialize prompt-correct VBench-Long inputs for v180."""
...
def prepare(run_root: Path, comparison_root: Path) -> dict:
    ...
    if not published_path.is_file() or not contract_path.is_file():
        raise ValueError("v180 generation must be audited before VBench prepare")
    ...
    if (
        ...
        or published.get("experiment") != "v180_rccp_fresh128_generation"
        or contract.get("experiment") != "v180_rccp_fresh128_generation"
    ...
    if (
        ...
        or input_manifest["prompt_file_sha256"] != contract["prompt_file_sha256"]
    ):
        raise ValueError("v180 prompt mapping or hash drift")
    ...
    payload = {
        ...
        "prompt_items": [
            {"index": index, "source_index": source_indices[index], "text": prompts[index]}
            for index in range(PROMPT_COUNT)
        ],
