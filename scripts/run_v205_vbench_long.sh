#!/usr/bin/env bash
# VBench-Long, temporal safety, and local-motion evidence for v205.
set -euo pipefail
...
case "$ACTION" in
    ...
    *)
        echo "usage: bash scripts/run_v205_vbench_long.sh ACTION"
...
INPUT_MANIFEST="${V205_INPUT_MANIFEST:-$OUT_BASE/inputs/manifest.json}"
...
TEMPORAL_CSV="${V205_TEMPORAL_CSV:-$RUN_ROOT/metrics/temporal_diagnostics.csv}"
TEMPORAL_CONTRACT="${V205_TEMPORAL_CONTRACT:-$RUN_ROOT/metrics/temporal_diagnostics.contract.json}"
REPORT="$ANALYSIS_ROOT/v205_horizon_confirmation.json"
MOTION_ROOT="${V205_MOTION_ROOT:-$RUN_ROOT/motion}"
...
MOTION_REPORT="$ANALYSIS_ROOT/v205_continuous_motion.json"
...
compute_temporal() {
    ...
    [[ -s "$comparison" ]] || {
        echo "[error] missing v205 VBench manifest; run prepare"; exit 2;
    }
    activate_env
...
if (
    payload.get("experiment") != "v205_profile_disjoint_head_phase_horizon_vbench"
    or payload.get("confirmatory") is not True
    or int(payload.get("prompt_count", -1)) != 128
):
    raise SystemExit("wrong v205 comparison manifest")
for row in payload["methods"]:
    print(row["video_dir"])
PY
    )
    [[ "${#video_dirs[@]}" -ge 2 && "${#video_dirs[@]}" -le 3 ]] || {
        echo "[error] v205 requires SF plus one or two candidates"; exit 2;
    }
    "$PYTHON_BIN" "$ROOT/scripts/compute_temporal_jump_diagnostic.py" \
        "${video_dirs[@]}" --output "$TEMPORAL_CSV" \
        --expected-videos 128 --max-width "${V205_TEMPORAL_WIDTH:-256}" \
        --frame-step "${V205_TEMPORAL_FRAME_STEP:-8}" \
        --workers "${V205_TEMPORAL_WORKERS:-8}"
...
from pathlib import Path
payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
selected = payload.get("selected_v201_candidates") or []
if not selected:
    raise SystemExit("v205 comparison has no selected candidate")
print(selected[0])
PY
}
...
delegate_motion() {
    ...
    [[ -s "$COMPARISON_ROOT/comparison_manifest.json" ]] || {
        echo "[error] prepare v205 VBench comparison first"; exit 2;
    }
    ...
    TARGET=custom V193_SOURCE_RUN_ROOT="$RUN_ROOT" \
        ...
        QUALITY_REPORT="$REPORT" CANDIDATE="$candidate" CONTROLS=sf_native \
...
if [[ "$ACTION" == "prepare" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT/scripts/prepare_v205_vbench_comparison.py" \
...
if [[ "$ACTION" == "decision" ]]; then
    ...
    "$PYTHON_BIN" - "$REPORT" "$MOTION_REPORT" <<'PY'
import json, sys
from pathlib import Path
report = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
print(f"[v205-decision] {report['recommendation']}")
print("confirmed=" + ",".join(report["confirmed_candidates"]))
print("late_half=" + ",".join(report["long_horizon_supported_candidates"]))
print("manual_review_required=false")
motion = Path(sys.argv[2])
if motion.is_file():
    payload = json.loads(motion.read_text(encoding="utf-8"))
    print(f"motion={payload['recommendation']}")
for row in report["targeted_review_queue"]:
    print(f"optional_review=p{row['prompt_index']}:source{row['source_index']}")
PY
    exit $?
fi
...
if [[ "$ACTION" == "motion-analyze" ]]; then
    ...
    for path in "$REPORT" "$MOTION_CSV" "$MOTION_CONTRACT"; do
        [[ -s "$path" ]] || { echo "[error] missing v205 motion input: $path"; exit 2; }
    done
    activate_env
    "$PYTHON_BIN" "$ROOT/scripts/analyze_v205_continuous_motion.py" \
...
if [[ "$ACTION" == "package" ]]; then
    ...
    archive="$RUN_ROOT/v205_evaluation_small_artifacts.tar.gz"
    optional=()
    if [[ -d "$MOTION_ROOT" && "$MOTION_ROOT" == "$RUN_ROOT"/* ]]; then
        optional+=("${MOTION_ROOT#"$RUN_ROOT/"}")
    elif [[ -d "$MOTION_ROOT" ]]; then
        echo "[v205-package] skip external motion root: $MOTION_ROOT" >&2
    fi
...
TORCH_HUB_DIR="${V205_TORCH_HUB_DIR:-$ROOT/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V205_RUNTIME_HOME:-$ROOT/runs/_model_cache/dreamsim_home}"
if [[ ( "$ACTION" == "eval" || "$ACTION" == "resume-missing" ) && \
      "${V205_LOCAL_MODELS:-1}" == "1" ]]; then
...
"$PYTHON_BIN" "$ROOT/scripts/run_v205_vbench_long.py" "$PYTHON_ACTION" \
    ...
    --analysis-stem v205_vbench_analysis \
    --summary-title "v205 Profiling-Disjoint 128 Horizon Confirmation" \
    "${EXTRA_ARGS[@]}"
...
if [[ "$ACTION" == "collect" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT/scripts/analyze_v205_horizon_confirmation.py" \
        --comparison-root "$COMPARISON_ROOT" \
