#!/usr/bin/env python3
"""Paired development-only decision for the frozen v211 LPHC screen8."""
...
from prepare_v211_lphc import METHODS, SCREEN_FRAMES, SOURCE_INDICES, effective_seed
from prepare_v211_vbench_comparison import DIMENSIONS, EXPERIMENT
...
CANDIDATES = ("lphc_sink1_e1_a002_r4", "lphc_sink1_e1_a010_r4")
PRIMARY_METRICS = (
...
def verify_manifest_provenance(manifest: dict) -> bool:
    ...
    try:
        ...
        return False
    smoke_audits = smoke.get("audits") or {}
    smoke_completions = smoke.get("completions") or {}
    generation_digest = source.get("generation_manifest_sha256")
    if (
        generation.get("source_commit") != manifest.get("source_generation_commit")
        or generation.get("runtime_paths")
        != manifest.get("source_generation_runtime_sha256")
        or gate0.get("stage") != "gate0"
        or gate0.get("source_commit") != generation.get("source_commit")
        or gate0.get("input_manifest_sha256") != generation_digest
        or gate0.get("pass") is not True
        or gate0.get("errors") != []
        or smoke.get("stage") != "smoke"
        or smoke.get("source_commit") != generation.get("source_commit")
        or smoke.get("input_manifest_sha256") != generation_digest
        or smoke.get("source_index") != SOURCE_INDICES[0]
        or smoke.get("pass") is not True
        or smoke.get("errors") != []
        or tuple(smoke_audits) != CANDIDATES
        or tuple(smoke_completions) != CANDIDATES
        or any(smoke_audits[method].get("pass") is not True for method in CANDIDATES)
        or any(
            (smoke_audits[method].get("totals") or {}).get("random") != 0
...
def runtime_provenance_checks(manifest: dict, summary: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "source_media_audit": bool(
            source_media_audit.get("video_count") == len(METHODS) * PROMPT_COUNT
        ),
...
def validate_manifest_summary(manifest: dict, summary: dict) -> dict:
    ...
    checks = {
        ...
        "primary_controls": (
            ...
            and "capacity_context" not in manifest
        ),
    ...
    if not checks["pass"]:
        ...
        raise ValueError(f"incomplete/mixed v211 analysis inputs: {', '.join(failed)}")
    return checks
...
def comparison(
    ...
    if len(matches) != 1:
        raise ValueError(
            f"missing v211 comparison {candidate}/{control}/{metric}/{window}"
        )
    return matches[0]
...
def analyze_from_rows(
    ...
    *,
    ...
    methods = tuple(str(row.get("key", "")) for row in manifest.get("methods") or ())
    if methods != METHODS:
        raise ValueError("v211 analysis requires the frozen four-method order")
    ...
    if prompt_count != PROMPT_COUNT or tuple(rows_by_window) != tuple(WINDOWS):
        raise ValueError("v211 analysis requires the frozen screen8 window grid")
    expected_keys = {(method, prompt) for method in methods for prompt in range(prompt_count)}
    for window, rows in rows_by_window.items():
        if set(rows) != expected_keys:
            raise ValueError(f"v211 {window} prompt grid is incomplete or mixed")
        for key in expected_keys:
            missing = set(ANALYSIS_METRICS) - set(rows[key])
            if missing:
                raise ValueError(f"v211 {window}/{key} lacks metrics: {sorted(missing)}")
    if set(temporal_rows) != expected_keys:
        raise ValueError("v211 temporal prompt grid is incomplete or mixed")
    ...
    for row in comparisons:
        if id(row) in primary_ids:
            ...
            row["q_value"] = None
            row["inferential_role"] = "development_window_context"
    ...
    return {
        ...
        "recommendation": (
            "recommend_selected_lphc_candidate_for_manual_review"
            if selected is not None
            else "stop_v211_no_eligible_lphc_candidate"
        ),
        "provenance_gate": {
        ...
        "claim_boundary": (
            ...
            "not support a paper claim. Dynamic Degree does not affect selection. "
            "At most one method may be recommended, and no extension campaign is "
...
def write_frozen(path: Path, payload: bytes) -> None:
    ...
    if path.exists():
        if path.read_bytes() != payload:
            raise RuntimeError(f"frozen v211 analysis artifact differs: {path}")
        return
...
def main() -> None:
    ...
    if not checks["pass"]:
        raise ValueError("v211 VBench part provenance drifted")
    ...
    output = output_dir / "v211_lphc_screen.json"
    write_frozen(
        output,
        (json.dumps(report, indent=2, sort_keys=True) + "\n").encode("utf-8"),
    )
    write_frozen(output.with_suffix(".md"), render(report).encode("utf-8"))
    write_comparisons(output_dir / "v211_lphc_screen_comparisons.csv", report["comparisons"])
    print(
        "[v211-analysis] "
        f"recommendation={report['recommendation']} "
