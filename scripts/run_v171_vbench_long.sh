...
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/v171_demand_gated_motion_moviebench16/full8}"
...
V170_MATCHED_METRICS="${V170_MATCHED_METRICS:-${ROOT}/runs/v170_matched_attribution_moviebench16/full8/analysis/v170_matched_metrics.json}"
DECISION_JSON="${DECISION_JSON:-${ANALYSIS_ROOT}/v171_corrected_metrics.json}"
...
if [[ "${ACTION}" == "prepare" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v171_vbench_comparison.py" \
    --run-root "${RUN_ROOT}" \
...
if [[ "${ACTION}" == "split" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v171_vbench_splits.py" \
    --comparison-root "${COMPARISON_ROOT}" \
    --vbench-root "${VBENCH_ROOT}" \
    --workers "${V171_SPLIT_WORKERS:-2}" \
    --node-rank "${NODE_RANK}" \
    --num-nodes "${NUM_NODES}"
  exit $?
fi
...
TORCH_HUB_DIR="${V171_TORCH_HUB_DIR:-${ROOT}/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V171_RUNTIME_HOME:-${ROOT}/runs/_model_cache/dreamsim_home}"
if [[ ( "${ACTION}" == "eval" || "${ACTION}" == "resume-missing" ) && \
      "${V171_LOCAL_MODELS:-1}" == "1" ]]; then
...
"${PYTHON_BIN}" "${ROOT}/scripts/run_v171_vbench_long.py" "${PYTHON_ACTION}" \
  ...
  --analysis-stem "v171_vbench_analysis" \
  --summary-title "v171 Demand-gated Motion VBench-Long Core-9" \
  "${EXTRA_ARGS[@]}"
