#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for v171."""
...
def analyze(payload: dict) -> dict:
    ...
    if tuple(rows) != METHODS or dimensions != DIMENSIONS or payload.get("missing"):
        raise ValueError("v171 VBench summary violates the frozen grid")
    for method in METHODS:
        row = rows[method]
        if set(row) != set(DIMENSIONS):
            raise ValueError(f"{method}: incomplete v171 VBench dimensions")
        if (
    ...
    comparisons = {
        candidate: {
            "exclusive_delta_vs_v166": {
                metric: derived[candidate][metric] - derived[V166][metric]
                for metric in EXCLUSIVE_GROUPS
            },
            "official_quality_delta_vs_v166": quality[candidate] - quality[V166],
        }
...
def configure_base() -> None:
    ...
    base.ANALYSIS_STEM = "v171_vbench_analysis"
    base.SUMMARY_TITLE = "v171 Demand-gated Motion VBench-Long Core-9"
