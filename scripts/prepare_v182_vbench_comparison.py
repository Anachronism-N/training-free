#!/usr/bin/env python3
"""Materialize prompt-correct VBench-Long inputs for v182 screen16."""
...
def link_or_validate(source: Path, target: Path) -> str:
    ...
    if target.exists() or target.is_symlink():
        if not target.samefile(source):
            raise RuntimeError(f"refusing mixed v182 VBench input: {target}")
        return "existing"
    try:
...
def write_frozen(path: Path, payload: dict) -> str:
    ...
    if path.exists() and path.read_bytes() != encoded:
        raise RuntimeError(f"frozen v182 VBench manifest differs: {path}")
...
def prepare(run_root: Path, comparison_root: Path) -> dict:
    ...
    if not published_path.is_file() or not contract_path.is_file():
        raise ValueError("v182 screen16 generation must be audited first")
    published = json.loads(published_path.read_text(encoding="utf-8"))
    contract = json.loads(contract_path.read_text(encoding="utf-8"))
    if (
        published.get("ok") is not True
        or published.get("experiment") != "v182_structured_coverage_generation"
        or published.get("scope") != "screen16"
        or contract.get("scope") != "screen16"
        or published.get("experiment_contract_sha256") != sha256(contract_path)
    ):
        raise ValueError("invalid or mixed v182 generation artifacts")
    methods = tuple(row["key"] for row in published["methods"])
    if methods != tuple(contract["methods"]):
        raise ValueError("v182 method order differs from generation contract")
    prompt_count = int(contract["prompt_count"])
    if prompt_count != 16 or contract["prompt_indices"] != list(range(16)):
        raise ValueError("v182 VBench accepts only the complete 16-prompt screen")
    prompts_path = Path(contract["prompt_file"])
    prompts = prompts_path.read_text(encoding="utf-8").splitlines()
    if len(prompts) != prompt_count or sha256(prompts_path) != contract["prompt_file_sha256"]:
        raise ValueError("v182 prompt suite hash drift")
    ...
    payload = {
        "version": 1,
        "experiment": "v182_structured_coverage_vbench_screen16",
        "development_only": True,
        "prompt_suite": "moviegen_qwen_diverse16",
