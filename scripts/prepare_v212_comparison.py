...
def validate_pairs(jobs: list[dict]) -> None:
...
def verify_published(repo: Path, comparison: Path, *, media: bool = True) -> dict:
    ...
    require_gate(out, generation)
    validate_pairs(data["jobs"])
...
def prepare(repo: Path, out: Path, vbench: Path) -> dict:
    ...
    for source in p.SOURCE_INDICES:
        for method in p.METHODS:
            row = load_done(out, data, "screen32", method, source)
...
if __name__ == "__main__":
    ...
    result = prepare(Path(__file__).resolve().parents[1], p.output_root(args.run_root), args.vbench_root.resolve())
    print(f"[v212-publish] validated={len(result['jobs'])} same_gpu_pairing=true")
