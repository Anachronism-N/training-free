...
case "$ACTION" in
    ...
    *)
        echo "usage: bash scripts/run_v187_vbench_long.sh {prepare|split|preflight|eval|resume-missing|status|collect|decision}"
        exit 2
        ;;
esac
...
INPUT_MANIFEST="${V187_INPUT_MANIFEST:-$OUT_BASE/inputs/manifest.json}"
...
if [[ "$ACTION" == "prepare" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT/scripts/prepare_v187_vbench_comparison.py" \
...
if [[ "$ACTION" == "decision" ]]; then
    ...
    "$PYTHON_BIN" - "$ANALYSIS_ROOT/v187_unseen128_confirmation.json" <<'PY'
import json
import sys
from pathlib import Path

payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
print(f"[v187-decision] {payload['recommendation']}")
print("schedule=" + payload["selected_schedule"])
print("operator=" + payload["selected_operator"])
print("benchmark_confirmed=" + str(payload["benchmark_advantage_confirmed"]).lower())
print("operator_attribution_confirmed=" + str(payload["operator_attribution_confirmed"]).lower())
...
TORCH_HUB_DIR="${V187_TORCH_HUB_DIR:-$ROOT/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V187_RUNTIME_HOME:-$ROOT/runs/_model_cache/dreamsim_home}"
if [[ ( "$ACTION" == "eval" || "$ACTION" == "resume-missing" ) && \
      "${V187_LOCAL_MODELS:-1}" == "1" ]]; then
...
"$PYTHON_BIN" "$ROOT/scripts/run_v174_vbench_long.py" "$PYTHON_ACTION" \
    ...
    --analysis-stem v187_vbench_analysis \
    --summary-title "v187 Unseen-128 Phase-Operator VBench-Long" \
    "${EXTRA_ARGS[@]}"

if [[ "$ACTION" == "collect" ]]; then
    "$PYTHON_BIN" "$ROOT/scripts/analyze_v187_unseen128_confirmation.py" \
        --comparison-root "$COMPARISON_ROOT" \
        --summary "$SUMMARY_ROOT/vbench_core9_summary.json" \
        --parts-root "$PARTS_ROOT" \
        --output "$ANALYSIS_ROOT/v187_unseen128_confirmation.json"
fi
