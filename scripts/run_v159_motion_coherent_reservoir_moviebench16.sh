...
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/v159_motion_coherent_reservoir_moviebench16/full8}"
NODE_RANK="${NODE_RANK:-0}"
NUM_NODES="${NUM_NODES:-4}"
GPU_LIST="${GPU_LIST:-0,1,2,3,4,5,6,7}"

DEFAULT_REUSE_ROOT="${ROOT}/runs/v157_layer_gated_moviebench16/full8"
if [[ -z "${V159_REUSE_V157_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V159_REUSE_V157_ROOT="${DEFAULT_REUSE_ROOT}"
fi
export V120_BASELINE_ONLY=0
export V120_OURS_ONLY=0

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v159_motion_coherent_reservoir_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
  ...
  --num-nodes "${NUM_NODES}"
  --gpu-list "${GPU_LIST}"
  --candidates "interleaved10_reservoir2_motionpair1,interleaved10_motionpair2,middle10_reservoir2_motionpair1,interleaved10_reservoir4_reference,middle10_reservoir4_reference,all_reservoir4_reference,all_recent8_reference"
)
...
case "${ACTION}" in
  ...
  blind)
    ...
    ;;
  package)
    package_path="${OUT_ROOT}/v159_diagnostics.tar.gz"
