#!/usr/bin/env python3
"""Run one source-bound v211 prompt in one Self-Forcing subprocess."""
...
def scrub_env(env: dict[str, str]) -> dict[str, str]:
    """Remove every inherited intervention control before applying v211 values."""
    return {
        key: value
...
def assert_authorized_node(
    ...
    *,
    ...
    actual = node_address or os.environ.get("V211_NODE_ADDRESS")
    allowed = manifest.get("authorized_nodes", [])
    if not actual:
        raise PermissionError("V211_NODE_ADDRESS is required for v211 launches")
    if actual not in allowed:
        raise PermissionError(f"node address {actual!r} is not in the exact v211 allowlist")
...
def resolve_method(manifest: dict, stage: str, method: str) -> tuple[str, dict]:
    ...
    if method not in METHODS:
        raise ValueError(f"invalid v211 method: {method}")
...
def quarantine_job(job: Path, output_root: Path, reason: str) -> Path | None:
    ...
    if not _inside(job, output_root):
        raise ValueError("job path escaped the v211 output root")
    ...
    if not _inside(target, output_root):
        raise ValueError("quarantine path escaped the v211 output root")
...
def validate_media(path: Path, latent_frames: int) -> dict:
    ...
    if shutil.which("ffprobe") is None:
        raise RuntimeError("ffprobe is required for the frozen v211 media audit")
    command = [
...
def lphc_environment(spec: dict, trace_path: Path, source_index: int | None = None) -> dict[str, str]:
    ...
    result = {
        ...
        "LPHC_ARCHIVE_FRAMES": str(int(spec.get("archive_frames", ARCHIVE_SIZE))),
...
def run_job(
    ...
    *,
    ...
    node_address = assert_authorized_node(manifest, hostname=hostname)
    host = platform.node()
    config_key, spec = resolve_method(manifest, stage, method)
    if stage in {"gate0", "smoke"}:
        expected_node = manifest["authorized_nodes"][0]
    else:
        job_position = METHODS.index(method) * len(SOURCE_INDICES) + SOURCE_INDICES.index(source_index)
        expected_node = manifest["authorized_nodes"][job_position % len(manifest["authorized_nodes"])]
    if node_address != expected_node:
        raise PermissionError(
            f"v211 {stage} job is assigned to {expected_node}, not {node_address}"
        )
    if stage == "gate0":
        if source_index not in manifest["gate0"]["source_indices"]:
            raise ValueError("gate0 source index drift")
    elif source_index not in SOURCE_INDICES:
        raise ValueError("active generation is restricted to the frozen Screen8 source indices")
    if stage == "smoke" and (method not in SMOKE_METHODS or source_index != SOURCE_INDICES[0]):
        raise ValueError("smoke requires both frozen LPHC candidates on source 3")
    ...
    env.update(
        ...
        V211_CONTRACT_SHA256=stamp["input_manifest_sha256"],
        V211_SOURCE_INDEX=str(source_index),
        V211_EFFECTIVE_SEED=str(item["effective_seed"]),
    )
    ...
    if tensor_trace_dir is not None:
        env.update(
            ...
            SF_PARITY_RUN_KIND=f"v211_{method}",
    ...
    invocation = {
        ...
        "environment": {
            ...
            if key.startswith(("CUDA_", "PYTORCH_", "SF_PARITY_", "LPHC_", "V211_"))
        },
    }
    ...
    if stamp["requires_lphc_trace"]:
        if latent_frames % 3:
            raise RuntimeError("v211 latent frame count must be divisible by the 3-frame block size")
...
def main() -> None:
    ...
    print(f"[v211-worker] done {done['stamp']['stage']}/{done['stamp']['method']}/{done['stamp']['source_index']}")
