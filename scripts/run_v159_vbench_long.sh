...
if [[ "${ACTION}" == "prepare" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v159_vbench_comparison.py" \
...
if [[ "${ACTION}" == "split" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v159_vbench_splits.py" \
    --comparison-root "${COMPARISON_ROOT}" \
    --vbench-root "${VBENCH_ROOT}" \
    --workers "${V159_SPLIT_WORKERS:-2}" \
...
TORCH_HUB_DIR="${V159_TORCH_HUB_DIR:-${ROOT}/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V159_RUNTIME_HOME:-${ROOT}/runs/_model_cache/dreamsim_home}"
if [[ ( "${ACTION}" == "eval" || "${ACTION}" == "resume-missing" ) && \
      "${V159_LOCAL_MODELS:-1}" == "1" ]]; then
...
DIMENSIONS="${V159_VBENCH_DIMENSIONS:-}"
SUMMARY_STEM="vbench_long_summary"
ANALYSIS_STEM="v159_vbench_analysis"
SUMMARY_TITLE="v159 Motion-Recovery VBench-Long Summary"
if [[ "${ACTION}" == "resume-missing" ]]; then
  PYTHON_ACTION="eval-missing"
fi
if [[ "${ACTION}" == "collect-core" ]]; then
  PYTHON_ACTION="collect"
  DIMENSIONS="${V159_CORE_DIMENSIONS:-subject_consistency,background_consistency,temporal_flickering,motion_smoothness,overall_consistency,dynamic_degree,aesthetic_quality,imaging_quality,temporal_style}"
  SUMMARY_STEM="vbench_core9_summary"
  ANALYSIS_STEM="v159_vbench_core9_analysis"
  SUMMARY_TITLE="v159 Motion-Recovery VBench-Long Core-9 Summary"
fi
