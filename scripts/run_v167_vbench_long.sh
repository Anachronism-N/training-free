...
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/v167_state_conditioned_motion_moviebench16/full8}"
...
DECISION_JSON="${DECISION_JSON:-${ANALYSIS_ROOT}/v167_corrected_metrics.json}"
...
run_decision() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v167_corrected_metrics.py" \
...
if [[ "${ACTION}" == "prepare-review" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v167_minimal_review.py" \
...
if [[ "${ACTION}" == "prepare" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v167_vbench_comparison.py" \
    --run-root "${RUN_ROOT}" \
...
if [[ "${ACTION}" == "split" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v167_vbench_splits.py" \
    --comparison-root "${COMPARISON_ROOT}" \
    --vbench-root "${VBENCH_ROOT}" \
    --workers "${V167_SPLIT_WORKERS:-2}" \
    --node-rank "${NODE_RANK}" \
    --num-nodes "${NUM_NODES}"
  exit $?
fi
...
TORCH_HUB_DIR="${V167_TORCH_HUB_DIR:-${ROOT}/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V167_RUNTIME_HOME:-${ROOT}/runs/_model_cache/dreamsim_home}"
if [[ ( "${ACTION}" == "eval" || "${ACTION}" == "resume-missing" ) && \
      "${V167_LOCAL_MODELS:-1}" == "1" ]]; then
...
"${PYTHON_BIN}" "${ROOT}/scripts/run_v167_vbench_long.py" "${PYTHON_ACTION}" \
  ...
  --analysis-stem "v167_vbench_analysis" \
  --summary-title "v167 State-conditioned Motion VBench-Long Core-9" \
  "${EXTRA_ARGS[@]}"
