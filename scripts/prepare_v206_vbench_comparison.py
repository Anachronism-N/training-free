#!/usr/bin/env python3
"""Materialize one audited v206 scope as prompt-correct VBench inputs."""
...
from prepare_v206_horizon_robustness import scope_config, verify
...
def link_or_validate(source: Path, target: Path) -> str:
    ...
    if target.exists() or target.is_symlink():
        if not target.samefile(source):
            raise RuntimeError(f"refusing mixed v206 VBench input: {target}")
        return "existing"
    try:
...
def write_frozen(path: Path, payload: dict) -> str:
    ...
    if path.exists() and path.read_bytes() != encoded:
        raise RuntimeError(f"frozen v206 VBench manifest differs: {path}")
...
def prepare(
    ...
    if not published_path.is_file() or not contract_path.is_file():
        raise ValueError(f"v206 {scope_key} generation must be audited first")
    ...
    if (
        ...
        or published.get("experiment") != "v206_horizon_robustness_generation"
        ...
        or contract.get("experiment") != "v206_horizon_robustness_generation"
        ...
        raise ValueError(f"invalid or mixed v206 generation artifacts: {scope_key}")
    ...
    if (
        ...
        raise ValueError(f"v206 prompt contract drifted: {scope_key}")
    ...
    for method in methods:
        ...
        if unexpected:
            raise RuntimeError(
                f"refusing stale v206 VBench videos for {method}: {sorted(unexpected)}"
            )
        hashes = {}
        ...
        if {path.name for path in target_dir.glob("*.mp4")} != expected_targets:
            raise RuntimeError(f"incomplete v206 VBench target set: {method}")
    ...
    payload = {
        ...
        "prompt_positions_in_v205": scope["prompt_positions_in_v205"],
        ...
        "confirmed_v205_candidates": frozen["confirmed_v205_candidates"],
        "v205_positive_axes_to_replicate": frozen[
...
def main() -> None:
    ...
    report = prepare(
        args.run_root, args.comparison_root, args.input_manifest, args.scope
    )
    print(
        "[v206-vbench-prepare] "
