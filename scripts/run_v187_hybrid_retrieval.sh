#!/usr/bin/env bash
# v187: PF + retrieval hybrid on 30s/128 prompts
# Keeps PF's native -1/1/2 head classification (best_labels.csv) but replaces
# stride/merge middle for stable heads with 4-frame semantic-retrieval middle.
# Comparison target: v185 pf_native (cyclic/stride/merge) on identical prompts.
set -euo pipefail
...
case "$ACTION" in
    generate|status) ;;
    *)
        echo "usage: bash scripts/run_v187_hybrid_retrieval.sh ACTION"
...
PROMPTS="${V187_PROMPTS:-$ROOT/prompts/moviegen_128_full.txt}"
OUT_ROOT="${V187_OUT_ROOT:-$ROOT/runs/v187_hybrid_retrieval}"
...
run_shard() {
    ...
    local log="$OUT_ROOT/logs/shard$(printf '%02d' "$rank").log"
    ...
    (
        ...
        # KEY: PF's default best_labels.csv (-1/1/2) + hybrid retrieval middle.
        # No --pyramidkv_head_config_path (yaml default), no cache_compatibility_policy.
        python inference.py \
...
case "$ACTION" in
    generate)
        activate_env
        echo "[v187] method=pf_hybrid_retrieval prompts=$PROMPT_COUNT frames=$FRAMES node=$NODE_RANK"
        declare -a pids=()
        for slot in "${!GPUS[@]}"; do
        ...
        [[ "$failed" -eq 0 ]] || { echo "[error] v187 failed on node=$NODE_RANK"; exit 1; }
        ;;
    status)
...
print(f"v187 pf_hybrid_retrieval: videos={len(indices)}/{count} missing={len(missing)}")
if missing:
