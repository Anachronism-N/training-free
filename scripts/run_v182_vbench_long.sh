...
case "$ACTION" in
    ...
    *)
        echo "usage: bash scripts/run_v182_vbench_long.sh {prepare|split|preflight|eval|resume-missing|status|collect|decision}"
        exit 2
        ;;
esac

ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
RUN_ROOT="${RUN_ROOT:-$ROOT/runs/v182_structured_coverage/screen16}"
...
if [[ "$ACTION" == "prepare" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT/scripts/prepare_v182_vbench_comparison.py" \
...
if [[ "$ACTION" == "decision" ]]; then
    ...
    "$PYTHON_BIN" - "$ANALYSIS_ROOT/v182_structured_coverage.json" <<'PY'
import json
import sys
...
print(f"[v182-decision] {payload['decision']}")
print("promoted=" + ",".join(payload["promoted_for_operator_specific_reprofiling"]))
print("review_queue=" + ",".join(
    f"p{row['prompt_index']}:{row['candidate']}" for row in payload["targeted_review_queue"]
))
PY
    exit $?
fi
...
TORCH_HUB_DIR="${V182_TORCH_HUB_DIR:-$ROOT/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V182_RUNTIME_HOME:-$ROOT/runs/_model_cache/dreamsim_home}"
if [[ ( "$ACTION" == "eval" || "$ACTION" == "resume-missing" ) && \
      "${V182_LOCAL_MODELS:-1}" == "1" ]]; then
...
"$PYTHON_BIN" "$ROOT/scripts/run_v174_vbench_long.py" "$PYTHON_ACTION" \
    ...
    --analysis-stem v182_vbench_analysis \
    --summary-title "v182 Structured-Coverage VBench-Long (development16)" \
    "${EXTRA_ARGS[@]}"

if [[ "$ACTION" == "collect" ]]; then
    "$PYTHON_BIN" "$ROOT/scripts/analyze_v182_structured_coverage.py" \
        --comparison-root "$COMPARISON_ROOT" \
        --summary "$SUMMARY_ROOT/vbench_core9_summary.json" \
        --parts-root "$PARTS_ROOT" \
        --output "$ANALYSIS_ROOT/v182_structured_coverage.json"
fi
