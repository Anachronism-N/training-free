...
METHODS=(
  ...
  ours_middle10_reservoir2_multiscalepareto1
  ours_middle10_reservoir2_multiscaleconsensus1
)
...
run_mechanism() {
  "${PYTHON_BIN}" \
    ...
    --output "${METRICS}/cross_scale_consensus_trace.json"
}
...
run_screen() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v168_automated_screen.py" \
...
case "${ACTION}" in
  ...
  all)
    ...
    run_comprehensive
    run_screen
    echo "automatic screen complete; do not review videos until analyzed"
    ;;
  *)
