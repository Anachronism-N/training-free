#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for a frozen v211 comparison."""
...
def evaluation_source_provenance(
    ...
    if commit != expected_commit:
        raise ValueError(
            "v211 evaluation checkout commit drift: "
    ...
    if (
        ...
        raise ValueError("v211 evaluation runtime file hash drift")
    ...
    if status.returncode != 0 or status.stdout.strip():
        raise ValueError("v211 evaluation checkout must remain exactly clean")
    return {
...
def validate_vbench_root(manifest: dict[str, Any], vbench_root: Path) -> Path:
    ...
    if recorded != actual:
        raise ValueError(
            f"v211 VBench root drift: expected={recorded} actual={actual}"
        )
    return actual
...
def validate_source_media(
    ...
    for row in manifest.get("jobs") or ():
        ...
        if key in jobs:
            raise ValueError(f"duplicate v211 source media job: {key}")
        jobs[key] = row
    ...
    if only_method is not None and only_method not in methods:
        raise ValueError(f"unknown v211 source media method: {only_method}")
    audit_rows = []
    ...
    for method_row in method_rows:
        ...
        for prompt_index in range(prompt_count):
            ...
            if (
                ...
                raise ValueError(
                    f"v211 source media provenance drift: {method}:{prompt_index}"
                )
            try:
...
def analyze(payload: dict) -> dict:
    ...
    if tuple(rows) != METHODS or dimensions != DIMENSIONS or payload.get("missing"):
        raise ValueError("v211 VBench summary violates the frozen grid")
    for method in METHODS:
        if set(rows[method]) != set(DIMENSIONS):
            raise ValueError(f"{method}: incomplete v211 VBench dimensions")
        if abs(
...
def render(report: dict) -> str:
    lines = [
        "# v211 LPHC Screen8 VBench-Long Core-9",
        "",
        (
...
def comparison_root_from_argv() -> Path:
    return _require_v211_output_root(
        _path_option_from_argv("--comparison-root"), "comparison root"
    )


def validate_output_roots_from_argv() -> None:
    for option, label in (
        ("--parts-root", "parts root"),
        ("--summary-root", "summary root"),
...
def build_split_provenance(
    ...
    for prompt_index in range(prompt_count):
        ...
        if any(not path.is_file() or path.stat().st_size <= 0 for path in expected):
            raise ValueError(f"{method}: incomplete v211 split clips")
        if {path.name for path in folder.iterdir()} != {path.name for path in expected}:
            raise ValueError(f"{method}: mixed v211 split clips")
        clip_rows.extend(
            {
    ...
    if len(source_rows) != prompt_count or len(clip_rows) != (
        prompt_count * clips_per_video
    ):
        raise ValueError(f"{method}: invalid v211 split provenance cardinality")
    return {
        "version": 1,
        "comparison_manifest_sha256": base.sha256(manifest_path),
        "method": method,
...
def _write_frozen_split_provenance(path: Path, payload: dict[str, Any]) -> None:
    ...
    if path.exists():
        if path.read_bytes() != encoded:
            raise ValueError(f"frozen v211 split provenance differs: {path}")
        return
    descriptor, temporary_name = tempfile.mkstemp(
        dir=path.parent, prefix=f".{path.name}."
    )
...
def validate_campaign_split_provenance(
    ...
    try:
        ...
        raise ValueError("missing v211 campaign split provenance") from error
    except (OSError, json.JSONDecodeError) as error:
        raise ValueError("invalid v211 campaign split provenance") from error
    ...
    prompt_count = int(manifest["prompt_count"])
    clips_per_video = int(manifest["num_output_frames"]) // 8
    if method_count != EXPECTED_METHOD_COUNT or list(expected_methods) != methods:
        raise ValueError(
            f"v211 campaign split provenance requires exactly {EXPECTED_METHOD_COUNT} methods"
        )
    aggregate_rows = [
        {
            "method": method,
    ...
    expected_campaign = {
        ...
        "source_count": method_count * prompt_count,
        "clip_count": method_count * prompt_count * clips_per_video,
    ...
    if (
        ...
        raise ValueError("v211 campaign split provenance drift")
    return {
...
def validate_split_provenance(
    ...
    *,
    ...
    if len(aggregate_rows) != expected_methods:
        raise ValueError(
            f"v211 split provenance requires exactly {expected_methods} methods"
        )
    report = {
        "version": 1,
        "method_count": expected_methods,
        "source_count": expected_methods * prompt_count,
        "clip_count": expected_methods * prompt_count * clips_per_video,
        "aggregate_digest": _canonical_digest(aggregate_rows),
...
def _job_lease_path(parts_root: Path, method: str, dimension: str) -> Path:
    ...
    return parts_root / ".v211_job_leases" / f"{digest}.lock"
...
def configure() -> dict:
    ...
    validate_output_roots_from_argv()
    ...
    if (
        ...
        raise ValueError("invalid v211 VBench comparison contract")
    METHODS = methods
    PROMPT_COUNT = prompt_count
    EXPERIMENT = experiment
    base.RUN_LABEL = "v211_lphc_screen8"
    base.SUMMARY_EXPERIMENT = experiment
    base.ANALYSIS_STEM = "v211_vbench_analysis"
    base.SUMMARY_TITLE = "v211 LPHC Screen8 VBench-Long Core-9"
