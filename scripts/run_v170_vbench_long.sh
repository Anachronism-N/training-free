...
if [[ "${ACTION}" != "prepare" && "${ACTION}" != "split" && \
      ...
      "${ACTION}" != "collect" && "${ACTION}" != "decide" ]]; then
  echo "usage: $0 {prepare|split|preflight|eval|resume-missing|status|collect|decide}" >&2
  exit 2
fi

ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/v170_matched_attribution_moviebench16/full8}"
...
MECHANISM_TRACE="${MECHANISM_TRACE:-${RUN_ROOT}/automated_screen/full_layer_trace.json}"
REPLICA_HASHES="${REPLICA_HASHES:-${RUN_ROOT}/automated_screen/replica_hashes.json}"
EXPERIMENT_CONTRACT="${EXPERIMENT_CONTRACT:-${RUN_ROOT}/contracts/experiment.json}"
DECISION_JSON="${DECISION_JSON:-${ANALYSIS_ROOT}/v170_matched_metrics.json}"
...
run_decision() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v170_matched_metrics.py" \
    --vbench-parts-root "${PARTS_ROOT}" \
    --vbench-summary "${SUMMARY_ROOT}/vbench_core9_summary.json" \
    --mechanism-trace "${MECHANISM_TRACE}" \
    --replica-hashes "${REPLICA_HASHES}" \
...
if [[ "${ACTION}" == "prepare" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v170_vbench_comparison.py" \
    --run-root "${RUN_ROOT}" \
...
if [[ "${ACTION}" == "split" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v170_vbench_splits.py" \
    --comparison-root "${COMPARISON_ROOT}" \
    --vbench-root "${VBENCH_ROOT}" \
    --workers "${V170_SPLIT_WORKERS:-2}" \
    --node-rank "${NODE_RANK}" \
    --num-nodes "${NUM_NODES}"
  exit $?
fi
...
TORCH_HUB_DIR="${V170_TORCH_HUB_DIR:-${ROOT}/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V170_RUNTIME_HOME:-${ROOT}/runs/_model_cache/dreamsim_home}"
if [[ ( "${ACTION}" == "eval" || "${ACTION}" == "resume-missing" ) && \
      "${V170_LOCAL_MODELS:-1}" == "1" ]]; then
...
"${PYTHON_BIN}" "${ROOT}/scripts/run_v170_vbench_long.py" "${PYTHON_ACTION}" \
  ...
  --analysis-stem "v170_vbench_analysis" \
  --summary-title "v170 Matched Attribution VBench-Long Core-9" \
  "${EXTRA_ARGS[@]}"
