#!/usr/bin/env bash
# Conditional seed and duration robustness runs for the v191-frozen route.
set -euo pipefail
...
case "$ACTION" in
    ...
    *)
        echo "usage: bash scripts/run_v192_head_phase_robustness_32gpu.sh ACTION"
...
V191_ROOT="${V191_OUT_ROOT:-$ROOT/runs/v191_head_phase_confirmation}"
V191_INPUT="${V191_INPUT_MANIFEST:-$V191_ROOT/inputs/manifest.json}"
V191_DECISION="${V191_DECISION:-$V191_ROOT/confirm128/analysis/v191_head_phase_confirmation.json}"
...
SMOKE_PROMPT_INDEX="${V192_SMOKE_PROMPT_INDEX:-0}"
FORCE="${FORCE:-0}"
...
for method in "${REQUESTED_METHODS[@]}"; do
    case ",$ALL_METHODS," in
        *",$method,"*) ;;
        *) echo "[error] unsupported v192 method: $method"; exit 2 ;;
    esac
    [[ -z "${METHOD_SEEN[$method]:-}" ]] || {
        echo "[error] duplicate v192 method: $method"; exit 2;
    }
...
prepare() {
    ...
    activate_env
    python "$ROOT/scripts/prepare_v192_head_phase_robustness.py" prepare \
        --v191-decision "$V191_DECISION" \
        --v191-input-manifest "$V191_INPUT" \
...
preflight() {
    ...
    python "$ROOT/scripts/prepare_v192_head_phase_robustness.py" verify \
        --manifest "$MANIFEST"
...
if row is None or sys.argv[3] not in row:
    raise SystemExit(f"missing v192 scope field: {sys.argv[2]}:{sys.argv[3]}")
...
run_shard() {
    ...
    if [[ "$FORCE" != "1" && -s "$marker" ]] && \
       shard_complete "$raw_dir" "$prompt_count" "$rank" "$stride"; then
        echo "[v192-skip] scope=$scope method=$method rank=$rank"
        return
    fi
...
smoke() {
    ...
    preflight
    local prompt_count run_root cursor method slot pid failed
    prompt_count="$(scope_field "$SCOPE" prompt_count)"
    [[ "$SMOKE_PROMPT_INDEX" -ge 0 && "$SMOKE_PROMPT_INDEX" -lt "$prompt_count" ]] || {
        echo "[error] V192_SMOKE_PROMPT_INDEX is outside $SCOPE"; exit 2;
    }
    ...
    while [[ "$cursor" -lt "${#REQUESTED_METHODS[@]}" ]]; do
        ...
        [[ "$failed" -eq 0 ]] || { echo "[error] v192 smoke failed"; exit 1; }
...
generate_scope() {
    ...
    [[ "$NUM_NODES" -eq 4 && "$GPUS_PER_NODE" -eq 8 && "$WORLD_SHARDS" -eq 32 ]] || {
        echo "[error] v192 generation is frozen to 4 nodes x 8 GPUs"; exit 2;
    }
    ...
    for ((offset=0; offset<count; offset++)); do
        ...
        echo "[v192-method] scope=$scope method=$method node=$NODE_RANK order_slot=$offset"
        local -a pids=()
        for slot in "${!GPUS[@]}"; do
        ...
        for pid in "${pids[@]}"; do wait "$pid" || failed=1; done
        [[ "$failed" -eq 0 ]] || {
            echo "[error] v192 scope=$scope method=$method failed on node=$NODE_RANK"
            exit 1
        }
    done
}
...
audit_scope() {
    ...
    python "$ROOT/scripts/audit_v192_head_phase_robustness.py" "${args[@]}"
}

status() {
    python - "$OUT_BASE" "$MANIFEST" "$ALL_METHODS" <<'PY'
import json, sys
from pathlib import Path
...
package() {
    ...
    local target="$OUT_BASE/v192_head_phase_robustness_diagnostics.tar.gz"
    tar -C "$OUT_BASE" -czf "$target" inputs scopes/*/contracts scopes/*/audits \
        ...
        scopes/*/logs scopes/*/traces analysis
    echo "$target"
}
