#!/usr/bin/env python3
"""Freeze the source-bound v211 LPHC evaluation contract."""
...
EXPERIMENT = "v211_lphc_sink_followup"
IMMUTABLE_OUTPUT_MARKERS = ("v209", "v210")
...
DEV32_SOURCE_INDICES = tuple(range(3, 128, 4))
SCREEN8_SOURCE_INDICES = DEV32_SOURCE_INDICES[::4]
SOURCE_INDICES = SCREEN8_SOURCE_INDICES
BASE_SEED = 21100
SCREEN_FRAMES = 120
GATE_FRAMES = 30
ARCHIVE_SIZE = 12
LOCAL_RECENT_FRAMES = 20
LPHC_PROTOCOL = "v211"
...
SMOKE_METHODS = ("lphc_sink1_e1_a002_r4", "lphc_sink1_e1_a010_r4")
METHOD_SPECS = {
    "sf_fifo21": {"local_attn_size": 21, "sink_size": 0, "lphc": False},
    "sf_sink1_21": {"local_attn_size": 21, "sink_size": 1, "lphc": False},
    "lphc_sink1_e1_a002_r4": {
        "local_attn_size": 21, "sink_size": 1, "lphc": True,
        "phase": "e1", "alpha": 0.02, "retrieval_mode": "correct",
        "archive_frames": 12, "history_frames": 4,
        "protocol": LPHC_PROTOCOL, "local_policy": LPHC_LOCAL_POLICY,
    },
    "lphc_sink1_e1_a010_r4": {
        "local_attn_size": 21, "sink_size": 1, "lphc": True,
        "phase": "e1", "alpha": 0.10, "retrieval_mode": "correct",
        "archive_frames": 12, "history_frames": 4,
        "protocol": LPHC_PROTOCOL, "local_policy": LPHC_LOCAL_POLICY,
    },
}
GATE0_METHODS = {
    "sf_sink1_21_native": {
        "config": "sf_sink1_21", "lphc": False, "phase": "e1", "alpha": 0.0,
        "retrieval_mode": "correct", "archive_frames": 12, "history_frames": 4,
        "protocol": LPHC_PROTOCOL, "local_policy": LPHC_LOCAL_POLICY,
    },
    "sink1_lphc_alpha0": {
        "config": "sf_sink1_21", "lphc": True, "phase": "e1", "alpha": 0.0,
        "retrieval_mode": "correct", "archive_frames": 12, "history_frames": 4,
        "protocol": LPHC_PROTOCOL, "local_policy": LPHC_LOCAL_POLICY,
    },
}
GATE0_SOURCE_INDICES = (3, 67)
RUNTIME_EXTENSIONS = {".py", ".yaml", ".yml", ".json", ".csv", ".cu", ".cpp", ".h", ".hpp", ".sh"}
V211_RUNTIME_FILES = (
    "src/lifecycle_kv/lphc.py",
    "scripts/prepare_v211_lphc.py",
    "scripts/run_v211_worker.py",
    "scripts/run_v211_lphc.py",
    "scripts/audit_v211_lphc_trace.py",
    "tests/test_v211_lphc_protocol.py",
)
WAN_REQUIRED_FILES = (
    "config.json",
...
def write_frozen(path: Path, payload: bytes) -> str:
    ...
    if path.exists() and path.read_bytes() != payload:
        raise RuntimeError(f"frozen v211 input differs: {path}")
    if not path.exists():
...
def require_clean_checkout(root: Path) -> None:
    ...
    if relevant:
        raise RuntimeError(
            "v211 prepare requires a clean source checkout; dirty paths: "
            + ", ".join(relevant[:8])
        )
...
def default_output_root(root: Path) -> Path:
    return root / "runs" / f"{EXPERIMENT}_{git_commit(root)}_sixnode"
...
def validate_output_root(path: Path) -> Path:
    ...
    try:
        relevant_parts = resolved.relative_to(repository).parts
    except ValueError:
        relevant_parts = resolved.parts
    lowered = "/".join(relevant_parts).lower()
    if any(marker in lowered for marker in IMMUTABLE_OUTPUT_MARKERS):
        raise ValueError(f"v209/v210 output roots are immutable and forbidden for v211: {resolved}")
    return resolved


def effective_seed(source_index: int) -> int:
    if source_index not in DEV32_SOURCE_INDICES:
        raise ValueError(f"source index is outside the frozen v211 DEV32 suite: {source_index}")
...
def _git_inventory(root: Path) -> set[str]:
    ...
    for name in V211_RUNTIME_FILES:
...
def verify_wan_inventory(root: Path, inventory: list[dict], *, check_hashes: bool) -> None:
    if not root.is_dir() or not inventory:
        raise ValueError("v211 Wan model is absent or has empty inventory")
    for row in inventory:
        path = root / row["relative_path"]
        if not path.is_file():
            raise ValueError(f"v211 Wan model file absent: {row['relative_path']}")
        stat = path.stat()
        if stat.st_size != row["bytes"] or stat.st_mtime_ns != row["mtime_ns"]:
            raise ValueError(f"v211 Wan model stamp drift: {row['relative_path']}")
        if check_hashes and sha256(path) != row["sha256"]:
            raise ValueError(f"v211 Wan model hash drift: {row['relative_path']}")
    ...
    *,
    ...
    for method in METHODS:
        ...
        config["lphc"] = {
            ...
            "archive_frames": int(spec.get("archive_frames", ARCHIVE_SIZE)),
            "history_frames": int(spec.get("history_frames", 0)),
            "protocol": spec.get("protocol", LPHC_PROTOCOL),
            "local_policy": spec.get("local_policy", LPHC_LOCAL_POLICY),
        }
    ...
    if cached is None and reuse_large_hashes_from is not None:
        ...
        if cached.get("experiment") != EXPERIMENT:
            raise ValueError("large-file hash provenance must come from this v211 experiment")
        hash_provenance = {
    ...
    payload = {
        ...
        "execution": {
            ...
            "ssh_port": 36000,
            "screen_assignment": "method-major jobs round-robin across frozen node ranks, then GPU slots",
        },
        "dev32_source_indices": list(DEV32_SOURCE_INDICES),
        "screen8_source_indices": list(SCREEN8_SOURCE_INDICES),
        "source_indices": list(SCREEN8_SOURCE_INDICES),
        "prompt_count": len(SCREEN8_SOURCE_INDICES),
        "prompt_items": prompt_items,
        "base_seed": BASE_SEED,
        "seed_policy": "effective_seed = 21100 + source_index",
        ...
        "lphc_defaults": {
            ...
            "protocol": LPHC_PROTOCOL,
            "local_policy": LPHC_LOCAL_POLICY,
        },
        "gate0": {
        ...
        "smoke": {
            "source_index": SCREEN8_SOURCE_INDICES[0],
            "methods": list(SMOKE_METHODS),
            "frames": SCREEN_FRAMES,
            "required_before_screen8": True,
        },
        "claim_boundary": "Immutable source-bound v211 Screen8. LPHC claims require sink1 gate0 tensor/cache equivalence and passing both history-4 candidate smoke trace audits; production attention only.",
    }
...
def verify(
    ...
    *,
    ...
    if (
        ...
        or data.get("dev32_source_indices") != list(DEV32_SOURCE_INDICES)
        or data.get("screen8_source_indices") != list(SCREEN8_SOURCE_INDICES)
        or data.get("source_indices") != list(SCREEN8_SOURCE_INDICES)
        or data.get("methods") != list(METHODS)
        or data.get("method_specs") != METHOD_SPECS
        or data.get("base_seed") != BASE_SEED
        or data.get("screen_frames") != SCREEN_FRAMES
        or data.get("gate_frames") != GATE_FRAMES
        or data.get("gate0", {}).get("source_indices") != list(GATE0_SOURCE_INDICES)
        or data.get("gate0", {}).get("modes") != GATE0_METHODS
        or data.get("smoke", {}).get("source_index") != SCREEN8_SOURCE_INDICES[0]
        or data.get("smoke", {}).get("methods") != list(SMOKE_METHODS)
    ):
        raise ValueError("invalid v211 manifest")
    validate_output_root(path)
    if data.get("source_commit") != git_commit(root):
        raise ValueError("v211 source commit drift; use a new output root")
    source = Path(data["prompt_source"]["path"])
    if sha256(source) != data["prompt_source"]["sha256"]:
        raise ValueError("v211 source prompt drift")
    for item in data["prompt_items"]:
        ...
        if sha256(prompt_path) != item["sha256"] or item["effective_seed"] != effective_seed(item["source_index"]):
            raise ValueError("v211 frozen prompt drift")
    for row in data["configs"].values():
        if sha256(Path(row["path"])) != row["sha256"]:
            raise ValueError("v211 config drift")
...
def main() -> None:
    ...
    parser.add_argument(
        "--authorized-nodes",
        default=os.environ.get("V211_AUTHORIZED_NODES", ",".join(AUTHORIZED_NODES)),
    ...
    print(f"[v211-prepare] root={validate_output_root(output)} prompts={data['prompt_count']} methods={len(METHODS)}")
