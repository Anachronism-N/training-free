...
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/v163_recency_regularized_state_motion_moviebench16/full8}"
NODE_RANK="${NODE_RANK:-0}"
NUM_NODES="${NUM_NODES:-4}"
GPU_LIST="${GPU_LIST:-0,1,2,3,4,5,6,7}"

DEFAULT_REUSE_ROOT="${ROOT}/runs/v161_state_matched_motion_moviebench16/full8"
if [[ -z "${V163_REUSE_V161_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V163_REUSE_V161_ROOT="${DEFAULT_REUSE_ROOT}"
fi
export V120_BASELINE_ONLY=0
export V120_OURS_ONLY=0

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v163_recency_regularized_state_motion_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
  ...
  --num-nodes "${NUM_NODES}"
  --gpu-list "${GPU_LIST}"
  --candidates "middle10_reservoir2_stateage12motionpair1,middle10_reservoir2_statebalancedmotionpair1,middle10_reservoir2_statemotionpair1_reference,middle10_reservoir2_freshmotionpair1_reference,middle10_reservoir4_reference"
)
...
case "${ACTION}" in
  ...
  mechanism)
    "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v163_recency_trace.py" \
      ...
      --output "${OUT_ROOT}/automated_screen/recency_trace.json"
    ;;
  status)
    ...
    ;;
  package)
    package_path="${OUT_ROOT}/v163_diagnostics.tar.gz"
