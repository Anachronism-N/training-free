#!/usr/bin/env python3
"""Prepare one blinded 12-video adaptive review wave for v163."""
...
from analyze_v163_automated_screen import METHODS, REVIEW_METHODS
...
PROMPT_COUNT = 16
RANDOM_SEED = 1632026
SOURCE_EXPERIMENT = "v163_statemotion_strict"
RUN_DIR_NAME = "v163_statemotion_strict_moviebench16"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-root",
        type=Path,
        default=ROOT / "runs" / RUN_DIR_NAME / "full8",
    )
    ...
    for prompt_index in prompt_indices:
        ...
        for slot, method in enumerate(methods):
            ...
            if not source.is_file():
                raise ValueError(f"missing v163 video: {source}")
            code = hashlib.sha256(
...
def main() -> None:
    ...
    if (
        ...
        raise ValueError("v163 adaptive-review manifests violate the contract")
    ...
    base.write_frozen(
        output_root
        / "reviewer"
        / f"v163_wave{args.wave}_review_sheet.csv",
        buffer.getvalue().encode("utf-8"),
    )
    key = {
        "version": 1,
        "experiment": "v163_adaptive_blind_review",
        "wave": args.wave,
        "seed": args.seed + args.wave,
        "methods": list(REVIEW_METHODS),
        "prompt_indices": prompt_indices,
    ...
    base.write_frozen(
        output_root
        / "private"
        / f"v163_wave{args.wave}_blind_key.json",
        base.canonical_json(key),
    )
    instructions = f"""# v163 Adaptive Blind Review: Wave {args.wave}
...
"""
    ...
    print(
        f"[v163-review] wave={args.wave} videos={len(key_rows)} "
