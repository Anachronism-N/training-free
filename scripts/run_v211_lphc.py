#!/usr/bin/env python3
"""Prepare, gate, smoke-audit, and run the source-bound v211 LPHC screen."""
...
REMOTE_SOURCE_BASE = Path("/tmp/training-free-v211-sources")
...
def write_decision(path: Path, report: dict) -> None:
    ...
    if path.exists() and path.read_bytes() != payload:
        raise RuntimeError(f"frozen v211 decision differs: {path}; recover or use a new output root")
    if not path.exists():
        path.write_bytes(payload)


def valid_decision(path: Path, stage: str, manifest_path: Path, manifest: dict) -> bool:
    if not path.is_file():
        return False
    try:
    ...
    if stage == "gate0":
        ...
        sequence = [
            (row.get("source_index"), row.get("mode"))
            for row in report.get("sequence", ())
        ]
        pairs = report.get("pairs") or ()
        return bool(
            report.get("attention") == "production"
    ...
    if (
        report.get("source_index") != source_index
        or report.get("latent_frames") != manifest["screen_frames"]
    ...
    return all(
        done_matches(
            completions[method],
            make_stamp(manifest, manifest_path, "smoke", method, source_index),
        )
...
def require_decision(output_root: Path, stage: str, manifest_path: Path, manifest: dict) -> dict:
    ...
    if not valid_decision(path, stage, manifest_path, manifest):
...
def launch_worker(repo_root: Path, output_root: Path, stage: str, method: str, source_index: int, gpu: str) -> None:
    command = [
        sys.executable,
        str(repo_root / "scripts" / "run_v211_worker.py"),
...
def run_gate0(repo_root: Path, output_root: Path, manifest_path: Path, manifest: dict, gpu: str) -> dict:
    ...
    for source_index in GATE0_SOURCE_INDICES:
        native = load_done(
            output_root, manifest_path, manifest, "gate0", "sf_sink1_21_native", source_index
        )
        alpha0 = load_done(
            output_root, manifest_path, manifest, "gate0", "sink1_lphc_alpha0", source_index
        )
        ...
        if not alpha0["trace"].get("present") or not trace_path.is_file():
            ...
            audit = audit_trace(
                trace_path, 0.0, expect_second_attention=False, phase="e1",
                expected_history_budget=4,
                expected_blocks=manifest["gate_frames"] // 3, expected_layers=30,
            )
        if not audit["pass"]:
...
def run_smoke(repo_root: Path, output_root: Path, manifest_path: Path, manifest: dict, gpu: str) -> dict:
    ...
    for method in SMOKE_METHODS:
        ...
        if not done["trace"].get("present") or not trace_path.is_file():
            ...
            audit = audit_trace(
                ...
                expect_second_attention=True, phase="e1",
                expected_history_budget=int(spec["history_frames"]),
                expected_blocks=manifest["screen_frames"] // 3,
                expected_layers=30,
            )
        audits[method] = audit
    ...
    report = {
        ...
        "pass": (
            not errors
            and tuple(audits) == SMOKE_METHODS
            and tuple(completions) == SMOKE_METHODS
        ),
    }
...
def screen_assignments(node_rank: int, gpus: tuple[str, ...], num_nodes: int = 6) -> dict[str, list[tuple[str, int]]]:
    if num_nodes != 6 or not 0 <= node_rank < num_nodes:
        raise ValueError("v211 screen requires node ranks 0..5 of exactly six nodes")
    if tuple(gpus) != tuple(str(index) for index in range(8)):
        raise ValueError("v211 screen requires the frozen GPU slots 0..7 on every node")
    jobs = [(method, source_index) for method in METHODS for source_index in SOURCE_INDICES]
    result = {gpu: [] for gpu in gpus}
    for position, job in enumerate(jobs):
        assigned_rank = position % num_nodes
        assigned_gpu = gpus[(position // num_nodes) % len(gpus)]
        if assigned_rank == node_rank:
            result[assigned_gpu].append(job)
    return result
...
def remote_repo_root(manifest: dict) -> Path:
    ...
    if len(commit) != 40 or any(char not in "0123456789abcdef" for char in commit):
        raise ValueError("invalid source commit in v211 manifest")
    ...
    for rank, node in enumerate(nodes):
        remote = shlex.join([
            ...
            " && ".join([
                ...
                f"export V211_NODE_ADDRESS={shlex.quote(node)} NODE_RANK={rank} NUM_NODES=6 GPU_LIST={shlex.quote(gpu_list)}",
                shlex.join([
                    "python", "scripts/run_v211_lphc.py", "screen8",
                    "--repo-root", str(repo_root),
                    "--output-root", str(output_root),
...
def status(output_root: Path, manifest_path: Path, manifest: dict) -> dict:
    result = {
        "gate0_decision": valid_decision(decision_path(output_root, "gate0"), "gate0", manifest_path, manifest),
        "smoke_decision": valid_decision(decision_path(output_root, "smoke"), "smoke", manifest_path, manifest),
        "stages": {},
    }
    stage_matrix = {
        "gate0": (tuple(GATE0_METHODS), GATE0_SOURCE_INDICES),
        "smoke": (SMOKE_METHODS, (SOURCE_INDICES[0],)),
...
def main() -> None:
    ...
    if args.num_nodes != 6 or not gpus or len(gpus) != len(set(gpus)):
        raise ValueError("v211 requires exactly six nodes and distinct non-empty GPUs")
    if args.action == "prepare":
        ...
        print(f"[v211] prepared {output_root}")
        return
