#!/usr/bin/env python3
"""Audit v167 state-conditioned motion retrieval from saved traces."""
...
STATE_RANK_MOTION = "ours_middle10_reservoir2_staterankmotion1"
DEFICIT_STATE_RANK_MOTION = (
    "ours_middle10_reservoir2_deficitstaterankmotion1"
)
METHODS = (STATE_RANK_MOTION, DEFICIT_STATE_RANK_MOTION)
EXPECTED_MODE = {
    STATE_RANK_MOTION: "state_ranked_multiscale_magnitude",
    DEFICIT_STATE_RANK_MOTION: (
...
def expected_selection(
    ...
    *,
    ...
    if scored:
        motion_selected, motion_values = max(
            scored,
            key=lambda item: (
        ...
        state_ordered = sorted(
            scored,
            key=lambda item: (
                float(item[0]["state_filter_similarity"]),
                int(item[0]["pair"][1]),
            ),
            reverse=True,
        )
        shortlist = state_ordered[:shortlist_size]
        state_selected, state_values = max(
            shortlist,
            key=lambda item: (
                float(item[1]["selection_score"]),
                float(item[1]["compatibility"]),
        ...
        newest, newest_values = max(
            scored,
            key=lambda item: int(item[0]["pair"][1]),
        )
        if (
            mode == "deficit_state_ranked_multiscale_magnitude"
            and not deficit_triggered
        ):
            selected, values = newest, newest_values
        else:
            selected, values = state_selected, state_values
        return {
            "pair": common.normalized_pair(selected["pair"]),
            "fallback": False,
            "score": values["score"],
            "motion_pair": common.normalized_pair(motion_selected["pair"]),
            "state_pair": common.normalized_pair(state_selected["pair"]),
            "shortlist": {
                tuple(common.normalized_pair(item[0]["pair"]))
...
def analyze_prompt(path: Path, *, method: str) -> dict:
    ...
    residual_state_scores: list[float] = []
    local_deficit_ratios: list[float] = []
    context_deficit_ratios: list[float] = []
    reason_counts: Counter[str] = Counter()
    retrieval_count = 0
    multi_candidate_count = 0
    changed_from_legacy_count = 0
    changed_from_motion_signature_count = 0
    state_rank_changed_from_motion_signature_count = 0
    deficit_ready_count = 0
    deficit_trigger_count = 0
    fallback_count = 0
    ...
    for row in rows:
        ...
        if retrieval.get("state_filter_mode") != "reference_residual_top_half":
            failures.append(
                f"line {row['line_number']}: state filter mode mismatch"
            )
        deficit_enabled = method == DEFICIT_STATE_RANK_MOTION
        if bool(retrieval.get("motion_deficit_gate_enabled")) != deficit_enabled:
            failures.append(
                f"line {row['line_number']}: deficit enable mismatch"
            )
        deficit = retrieval.get("motion_deficit", {})
        deficit_ready = bool(deficit.get("ready", False))
        local_ratio = deficit.get("local_ratio")
        context_ratio = deficit.get("context_ratio")
        recomputed_deficit = bool(
            deficit_ready
            and local_ratio is not None
            and context_ratio is not None
            and float(local_ratio) < 1.0
            and float(context_ratio) < 1.0
        )
        if bool(deficit.get("triggered", False)) != recomputed_deficit:
            failures.append(
                f"line {row['line_number']}: deficit rule mismatch"
            )
        expected_gate_trigger = deficit_enabled and recomputed_deficit
        if bool(retrieval.get("motion_deficit_gate_triggered")) != (
            expected_gate_trigger
        ):
            failures.append(
                f"line {row['line_number']}: deficit trigger mismatch"
            )
        deficit_ready_count += int(deficit_ready)
        deficit_trigger_count += int(expected_gate_trigger)
        if local_ratio is not None:
            local_deficit_ratios.append(float(local_ratio))
        if context_ratio is not None:
            context_deficit_ratios.append(float(context_ratio))
        ...
        for candidate in candidates:
            ...
            residual_state = candidate.get("residual_state_similarity")
            filter_state = candidate.get("state_filter_similarity")
            expected_filter_state = (
                residual_state
                if residual_state is not None
                else candidate.get("state_similarity")
            )
            if not close(filter_state, expected_filter_state):
                failures.append(
                    f"line {row['line_number']}: residual-state filter "
                    "mismatch"
                )
            if residual_state is not None:
                residual_state_scores.append(float(residual_state))
        ...
        expected = expected_selection(
            candidates,
            mode=mode,
            deficit_triggered=expected_gate_trigger,
        )
        state_ordered = sorted(
            [
                candidate
                for candidate in candidates
                if recompute_candidate(candidate, mode=mode)["passing"]
            ],
            key=lambda candidate: (
                float(candidate["state_filter_similarity"]),
                int(candidate["pair"][1]),
            ),
            reverse=True,
        )
        rank_by_pair = {
            tuple(common.normalized_pair(candidate["pair"])): rank
            for rank, candidate in enumerate(state_ordered, start=1)
        }
        for candidate in candidates:
            pair_key = tuple(common.normalized_pair(candidate["pair"]))
            expected_rank = rank_by_pair.get(pair_key)
            expected_pass = pair_key in expected["shortlist"]
            if candidate.get("state_filter_rank") != expected_rank:
                failures.append(
                    f"line {row['line_number']}: state rank mismatch for "
                    f"{list(pair_key)}"
                )
            if bool(candidate.get("state_filter_pass")) != expected_pass:
                failures.append(
                    f"line {row['line_number']}: shortlist mismatch for "
                    f"{list(pair_key)}"
                )
        if int(retrieval.get("state_shortlist_count", -1)) != len(
            expected["shortlist"]
        ):
            failures.append(
                f"line {row['line_number']}: shortlist count mismatch"
            )
        ...
        motion_rows = retrieval.get("motion_signature_selected", [])
        motion_pair = (
            common.normalized_pair(motion_rows[0])
            if isinstance(motion_rows, list) and len(motion_rows) == 1
            else None
        )
        state_rows = retrieval.get("state_rank_selected", [])
        state_pair = (
            common.normalized_pair(state_rows[0])
            if isinstance(state_rows, list) and len(state_rows) == 1
            else None
        )
        if motion_pair != expected["motion_pair"]:
            failures.append(
                f"line {row['line_number']}: motion counterfactual mismatch"
            )
        if state_pair != expected["state_pair"]:
            failures.append(
                f"line {row['line_number']}: state counterfactual mismatch"
            )
        changed_from_motion = bool(
            selected_pair is not None
            and motion_pair is not None
            and selected_pair != motion_pair
        )
        state_changed_from_motion = bool(
            state_pair is not None
            and motion_pair is not None
            and state_pair != motion_pair
        )
        if bool(
        ...
        if bool(
            retrieval.get("state_rank_changed_from_motion_signature")
        ) != state_changed_from_motion:
            failures.append(
                f"line {row['line_number']}: state-rank change flag mismatch"
            )
        changed_from_motion_signature_count += int(changed_from_motion)
        state_rank_changed_from_motion_signature_count += int(
            state_changed_from_motion
        )
    ...
    return {
        ...
        "changed_from_motion_signature_count": (
            changed_from_motion_signature_count
        ),
        "state_rank_changed_from_motion_signature_count": (
            state_rank_changed_from_motion_signature_count
        ),
        "deficit_ready_count": deficit_ready_count,
        "deficit_trigger_count": deficit_trigger_count,
        ...
        "residual_state_scores": residual_state_scores,
        "local_deficit_ratios": local_deficit_ratios,
        "context_deficit_ratios": context_deficit_ratios,
...
def aggregate_method(prompts: list[dict], *, method: str) -> dict:
    ...
    aggregate = {
        ...
        "changed_from_motion_signature_count": sum(
            prompt["changed_from_motion_signature_count"] for prompt in prompts
        ),
        "state_rank_changed_from_motion_signature_count": sum(
            prompt["state_rank_changed_from_motion_signature_count"]
            for prompt in prompts
        ),
        "deficit_ready_count": sum(
            prompt["deficit_ready_count"] for prompt in prompts
        ),
        "deficit_trigger_count": sum(
            prompt["deficit_trigger_count"] for prompt in prompts
        ),
        ...
        "residual_state_similarity": common.distribution(
            flatten("residual_state_scores")
        ),
        "local_deficit_ratio": common.distribution(
            flatten("local_deficit_ratios")
        ),
        "context_deficit_ratio": common.distribution(
            flatten("context_deficit_ratios")
        ),
    ...
    state_filter_active = (
        aggregate["state_rank_changed_from_motion_signature_count"] > 0
    )
    deficit_active = (
        method != DEFICIT_STATE_RANK_MOTION
        or (
            aggregate["deficit_trigger_count"] > 0
            and aggregate["deficit_ready_count"]
            > aggregate["deficit_trigger_count"]
        )
    )
    aggregate["mechanism_gate"] = bool(
        aggregate["archive_size_max"] >= 2
        and aggregate["multi_candidate_count"] > 0
        and aggregate["changed_from_motion_signature_count"] > 0
        and state_filter_active
        and deficit_active
...
def write_markdown(path: Path, report: dict) -> None:
    lines = [
        ...
        "",
        "| Method | Gate | Changed vs motion-only | State-filter changes | "
        "Deficit triggers/ready | State residual p50 | Age p95 | Failures |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    str(row["changed_from_motion_signature_count"]),
                    str(
                        row[
                            "state_rank_changed_from_motion_signature_count"
                        ]
                    ),
                    f"{row['deficit_trigger_count']}/"
                    f"{row['deficit_ready_count']}",
                    str(row["residual_state_similarity"]["median"]),
...
def main() -> None:
    ...
    report = {
        "version": 1,
        "experiment": "v167_state_conditioned_motion_trace",
        "mechanism_gate": mechanism_gate,
        "gate_definition": (
            "motion-score, residual-state rank, top-half filter, deficit "
            "gate, counterfactual selection, and atomic-read recomputation; "
            "both deficit branches must execute"
        ),
    ...
    if failure_count:
        raise SystemExit(
            f"v167 trace contract failed with {failure_count} violations"
        )
    if not mechanism_gate:
        raise SystemExit(
            "v167 mechanism gate failed; video metrics are blocked"
        )
    print(
        json.dumps(
            {
