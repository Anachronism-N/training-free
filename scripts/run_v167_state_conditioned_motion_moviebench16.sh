...
RUN_NAME="v167_state_conditioned_motion_moviebench16"
SMOKE_PROMPT_INDEX="${V167_SMOKE_PROMPT_INDEX:-6}"
...
if [[ -z "${V167_REUSE_V166_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V167_REUSE_V166_ROOT="${DEFAULT_REUSE_ROOT}"
fi
export V120_BASELINE_ONLY=0
export V120_OURS_ONLY=0

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v167_state_conditioned_motion_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
  ...
  --num-nodes "${NUM_NODES}"
  --gpu-list "${GPU_LIST}"
  --candidates "middle10_reservoir2_directionmatch1,middle10_reservoir2_multiscalemotion1,middle10_reservoir2_statemotionpair1_reference,middle10_reservoir2_staterankmotion1,middle10_reservoir2_deficitstaterankmotion1"
)
...
case "${ACTION}" in
  smoke)
    ...
    V167_SMOKE_PROMPT_INDEX="${SMOKE_PROMPT_INDEX}" "${runner[@]}" generate
    ;;
  ...
  mechanism)
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/analyze_v167_state_conditioned_motion_trace.py" \
      --trace-dir "${OUT_ROOT}/traces" \
      --output "${OUT_ROOT}/automated_screen/state_conditioned_motion_trace.json"
    ;;
  status)
    ...
    ;;
  package)
    package_path="${OUT_ROOT}/v167_diagnostics.tar.gz"
