#!/usr/bin/env python3
"""Audit whether v161 state-conditioned motion retrieval changed cache reads."""
...
def write_markdown(path: Path, report: dict) -> None:
    ...
    lines = [
        "# v161 State-Matched Motion Trace Audit",
        "",
...
def main() -> None:
    ...
    if [row["prompt_index"] for row in prompts] != list(range(PROMPT_COUNT)):
        raise ValueError("v161 trace prompt coverage mismatch")
    all_selected_ages = [
    ...
    report = {
        "version": 1,
        "experiment": "v161_state_matched_motion_trace",
        "primary": PRIMARY,
    ...
    if contract_failures:
        raise SystemExit(
            f"v161 trace contract failed with {contract_failures} violations"
        )
