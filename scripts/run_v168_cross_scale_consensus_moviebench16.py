#!/usr/bin/env python3
"""Run the v168 cross-scale-consensus motion-recall experiment."""
...
import run_v167_state_conditioned_motion_moviebench16 as v167
...
EXPERIMENT = "v168_cross_scale_consensus_moviebench16"
PROMPT_COUNT = 16
DIRECTION_MATCH = v167.DIRECTION_MATCH
MULTISCALE_MOTION = v167.MULTISCALE_MOTION
STATE_RANK_MOTION = v167.STATE_RANK_MOTION
...
REUSE_METHODS = {
    ...
    STATE_RANK_MOTION: STATE_RANK_MOTION,
}
NEW_METHODS = {PARETO_MOTION, CONSENSUS_MOTION}
PRIMARY = PARETO_MOTION
_PARENT_RUN_TASK = runner.run_task


V168_CELLS = (
    Cell(
        "v168_middle10_reservoir2_directionmatch1",
        "v168_reference",
        "single",
    ...
    Cell(
        "v168_middle10_reservoir2_multiscalemotion1",
        "v168_reference",
        "single",
    ...
    Cell(
        "v168_middle10_reservoir2_staterankmotion1",
        "v168_reference",
        "single",
        support_policy="reservoir2_staterankmotion1",
        suppress_policy="recent8_sink1",
        map_key="middle10",
    ),
    Cell(
        "v168_middle10_reservoir2_multiscalepareto1",
        "v168_cross_scale",
        "single",
    ...
    Cell(
        "v168_middle10_reservoir2_multiscaleconsensus1",
        "v168_cross_scale",
        "single",
        support_policy="reservoir2_multiscaleconsensus1",
        suppress_policy="recent8_sink1",
        map_key="middle10",
    ),
)
...
def configure_parent_runner() -> None:
    ...
    runner.PUBLISHED_TAG = "v168"
    runner.RUN_LABEL = "v168"
    ...
    runner.DEFAULT_CANDIDATES = (
        ...
        "middle10_reservoir2_staterankmotion1",
        "middle10_reservoir2_multiscalepareto1",
        "middle10_reservoir2_multiscaleconsensus1",
    )
    runner._CELLS_BY_NAME.update({cell.name: cell for cell in V168_CELLS})
    runner._CANDIDATE_SPECS.update(
        {
            ...
            "middle10_reservoir2_staterankmotion1": (
                "v168_middle10_reservoir2_staterankmotion1",
                "v167_state_rank_reference",
            ),
            "middle10_reservoir2_multiscalepareto1": (
                "v168_middle10_reservoir2_multiscalepareto1",
                "newest_dominance_guarded_motion_recall",
            ),
            "middle10_reservoir2_multiscaleconsensus1": (
                "v168_middle10_reservoir2_multiscaleconsensus1",
                "strict_local_context_consensus_recall",
            ),
        }
    )
...
def maybe_filter_smoke_tasks(tasks):
    raw = os.environ.get("V168_SMOKE_PROMPT_INDEX")
    if raw is None or not raw.strip():
        return list(tasks)
    prompt_index = int(raw)
    if not 0 <= prompt_index < PROMPT_COUNT:
        raise ValueError("V168_SMOKE_PROMPT_INDEX must be within [0, 15]")
    selected = [
        task
        for task in tasks
    ...
    if len(selected) != len(NEW_METHODS):
        raise RuntimeError(
            f"smoke filter expected {len(NEW_METHODS)} tasks, got "
            f"{len(selected)}"
        )
    return selected


def v167_run_root() -> Path:
    default = ROOT / "runs" / v167.EXPERIMENT / "full8"
    return Path(os.environ.get("V168_REUSE_V167_ROOT", default)).resolve()


def load_v167_source(prompt_manifest: dict, *, pf_runtime: dict) -> dict:
    root = v167_run_root()
    published_path = root / "published_manifest.json"
    contract_path = root / "contracts" / "experiment.json"
    trace_path = (
        root / "automated_screen" / "state_conditioned_motion_trace.json"
    )
    metrics_path = root / "analysis" / "v167_corrected_metrics.json"
    for path in (published_path, contract_path, trace_path, metrics_path):
        if not path.is_file():
            raise ValueError(f"missing frozen v167 reuse source: {path}")
    ...
    state_trace = (
        trace.get("methods", {})
        .get(STATE_RANK_MOTION, {})
        .get("aggregate", {})
    )
    if (
        published.get("ok") is not True
        or published.get("experiment") != v167.EXPERIMENT
        ...
        or state_trace.get("mechanism_gate") is not True
        or metrics.get("experiment") != "v167_corrected_metric_analysis"
        or not required_sources.issubset(quality)
    ):
        raise ValueError("v167 source does not match the frozen v168 contract")
    ...
    for target, source_method in REUSE_METHODS.items():
        ...
        if (
            ...
            raise ValueError(f"incomplete v167 reuse source: {source_method}")
        sources[target] = {
    ...
    return {
        ...
        "experiment_contract_sha256": text_sha256_lf(contract_path),
        "experiment_contract_raw_sha256": sha256(contract_path),
        "mechanism_trace": str(trace_path),
        "mechanism_trace_sha256": text_sha256_lf(trace_path),
...
def run_reused_task(
    ...
    *,
    ...
    reuse = args.v167_source["sources"][method.key]
    ...
    write_frozen(
        marker,
        {
            ...
            "reuse_manifest_sha256": args.v167_source[
    ...
    contract.update(
        {
            ...
            "motion_signature": {
                "descriptor": (
                    "existing layer-shared clean-KV frame descriptor; no "
                    "new model, training, or feature extractor"
                ),
                "local_direction": "normalized delta z[last-1] to z[last]",
                "context_direction": (
                    "normalized delta z[first] to z[last] within the block"
                ),
                "local_magnitude_match": "min(norm_q,norm_m)/max(norm_q,norm_m)",
                "context_magnitude_match": (
                    "same ratio after dividing block displacement by steps"
                ),
                "local_component": "local cosine times local magnitude match",
                "context_component": (
                    "context cosine times context magnitude match"
                ),
                "v166_score": (
                    "mean(local cosine, context cosine) times geometric mean "
                    "of local and context magnitude matches"
                ),
                "direction_gate": "frozen v166 mean-direction floor 0.1",
                "tuned_parameters": [],
            },
            "cross_scale_selection": {
                "pareto": (
                    "let the v166 motion winner override the newest compatible "
                    "pair only when it is no worse on both local and context "
                    "motion components"
                ),
                "consensus": (
                    "recall an older pair only when local and context component "
                    "argmaxes select the same pair; otherwise use the newest "
                    "compatible pair"
                ),
                "tie_break": "newer pair",
                "numeric_tolerance": "1e-12 comparison tolerance only",
                "learned_gate": False,
                "absolute_selection_threshold": None,
            },
            "design": {
                "primary": PRIMARY,
                "new_methods": [PARETO_MOTION, CONSENSUS_MOTION],
                ...
                "isolated_test": (
                    "replace only v166 old-history selection with a "
                    "cross-scale evidence guard"
                ),
                "fixed_factors": (
            ...
            "evaluation": {
                "mechanism_gate": (
                    "recompute every component and selected pair; Pareto must "
                    "exercise recall and newest rejection, Consensus must "
                    "exercise agreement and conflict, and all reads stay atomic"
                ),
                "primary_comparison": MULTISCALE_MOTION,
                "secondary_comparisons": ["sf_native", STATE_RANK_MOTION],
                "automatic_first": (
                    "run trace audit, temporal diagnostics, corrected VBench "
                    "and paired bootstrap before at most two prompt reviews"
                ),
            },
            "claim_boundary": (
                "v168 is a 16-prompt adaptive development experiment and "
...
def main() -> None:
    ...
    args.v167_source = load_v167_source(
        prompt_manifest,
    ...
    if tuple(method.key for method in methods) != EXPECTED_METHOD_KEYS:
        raise SystemExit(
            "v168 requires the frozen six-method order: "
    ...
    if args.mode == "preflight":
        ...
        total_task_count = len(runner.all_tasks(methods))
        print(
            f"[v168-preflight] PASS node={args.node_rank}/{args.num_nodes} "
            f"methods={len(methods)} total_tasks={total_task_count} "
    ...
    if args.mode == "audit":
        ...
        print(
            f"[v168-audit] PASS methods={len(payload['methods'])} "
    ...
    if not summary["ok"]:
        raise SystemExit("\n".join(failures or ["v168 task count mismatch"]))
