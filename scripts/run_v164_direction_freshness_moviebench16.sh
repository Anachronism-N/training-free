...
RUN_NAME="v164_direction_freshness_moviebench16"
...
if [[ -z "${V164_REUSE_V161_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V164_REUSE_V161_ROOT="${DEFAULT_REUSE_ROOT}"
fi
export V120_BASELINE_ONLY=0
export V120_OURS_ONLY=0

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v164_direction_freshness_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
  ...
  --num-nodes "${NUM_NODES}"
  --gpu-list "${GPU_LIST}"
  --candidates "middle10_reservoir2_directionmatch1,middle10_reservoir2_directionfresh1,middle10_reservoir2_statemotionpair1_reference,middle10_reservoir4_reference,all_recent8_reference"
)
...
case "${ACTION}" in
  ...
  mechanism)
    "${PYTHON_BIN}" \
      ...
      --output \
        "${OUT_ROOT}/automated_screen/direction_freshness_trace.json"
    ;;
  status)
    ...
    ;;
  package)
    package_path="${OUT_ROOT}/v164_diagnostics.tar.gz"
