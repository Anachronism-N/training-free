...
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/v161_state_matched_motion_moviebench16/full8}"
NODE_RANK="${NODE_RANK:-0}"
NUM_NODES="${NUM_NODES:-4}"
GPU_LIST="${GPU_LIST:-0,1,2,3,4,5,6,7}"

DEFAULT_REUSE_ROOT="${ROOT}/runs/v160_fresh_motion_moviebench16/full8"
if [[ -z "${V161_REUSE_V160_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V161_REUSE_V160_ROOT="${DEFAULT_REUSE_ROOT}"
fi
export V120_BASELINE_ONLY=0
export V120_OURS_ONLY=0

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v161_state_matched_motion_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
  ...
  --num-nodes "${NUM_NODES}"
  --gpu-list "${GPU_LIST}"
  --candidates "middle10_reservoir2_statemotionpair1,middle10_reservoir2_freshmotionpair1_reference,middle10_reservoir2_motionpair1_reference,middle10_reservoir4_reference,all_recent8_reference"
)
...
case "${ACTION}" in
  ...
  status)
    ...
    ;;
  package)
    package_path="${OUT_ROOT}/v161_diagnostics.tar.gz"
