...
case "$ACTION" in
    vbench-*)
        SCOPE="$SCOPE" NODE_RANK="$RANK" NUM_NODES="$NODES" \
            GPU_LIST=0,1,2,3,4,5,6,7 bash scripts/run_v208_vbench_long.sh "${ACTION#vbench-}"
        ;;
    *)
        PF_CHECKPOINT="$checkpoint" NODE_RANK="$RANK" NUM_NODES="$NODES" \
            GPU_LIST=0,1,2,3,4,5,6,7 RUN_UNIT_TESTS=0 \
            bash scripts/run_v208_paper_confirmation_32gpu.sh "$ACTION"
        ;;
esac
