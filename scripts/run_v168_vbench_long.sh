...
if [[ "${ACTION}" != "prepare" && "${ACTION}" != "split" && \
  ...
  echo "usage: $0 {prepare|split|preflight|eval|resume-missing|status|collect|decide|prepare-review}" >&2
  exit 2
fi

ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/v168_cross_scale_consensus_moviebench16/full8}"
...
MECHANISM_TRACE="${MECHANISM_TRACE:-${RUN_ROOT}/automated_screen/cross_scale_consensus_trace.json}"
DECISION_JSON="${DECISION_JSON:-${ANALYSIS_ROOT}/v168_corrected_metrics.json}"
...
run_decision() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v168_corrected_metrics.py" \
...
if [[ "${ACTION}" == "prepare-review" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v168_minimal_review.py" \
...
if [[ "${ACTION}" == "prepare" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v168_vbench_comparison.py" \
    --run-root "${RUN_ROOT}" \
...
if [[ "${ACTION}" == "split" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v168_vbench_splits.py" \
    --comparison-root "${COMPARISON_ROOT}" \
    --vbench-root "${VBENCH_ROOT}" \
    --workers "${V168_SPLIT_WORKERS:-2}" \
    --node-rank "${NODE_RANK}" \
    --num-nodes "${NUM_NODES}"
  exit $?
fi
...
TORCH_HUB_DIR="${V168_TORCH_HUB_DIR:-${ROOT}/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V168_RUNTIME_HOME:-${ROOT}/runs/_model_cache/dreamsim_home}"
if [[ ( "${ACTION}" == "eval" || "${ACTION}" == "resume-missing" ) && \
      "${V168_LOCAL_MODELS:-1}" == "1" ]]; then
...
"${PYTHON_BIN}" "${ROOT}/scripts/run_v168_vbench_long.py" "${PYTHON_ACTION}" \
  ...
  --analysis-stem "v168_vbench_analysis" \
  --summary-title "v168 Cross-scale Consensus VBench-Long Core-9" \
  "${EXTRA_ARGS[@]}"
