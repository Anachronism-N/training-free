#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for v164."""
from __future__ import annotations

import run_v154_vbench_long as base
from prepare_v164_vbench_comparison import (
...
def analyze(payload: dict) -> dict:
    ...
    if tuple(rows) != METHODS or dimensions != DIMENSIONS or payload.get("missing"):
        raise ValueError("v164 VBench summary violates the frozen grid")
    derived = {}
    for method in METHODS:
        row = rows[method]
        if set(row) != set(DIMENSIONS):
            raise ValueError(f"{method}: incomplete v164 VBench dimensions")
        derived[method] = {
    ...
    return {
        ...
        "claim_boundary": (
            "VBench is an automatic development diagnostic; v164 is not a "
...
def configure_base() -> None:
    ...
    base.ANALYSIS_STEM = "v164_vbench_analysis"
    base.SUMMARY_TITLE = "v164 Direction/Freshness VBench-Long Core-9"
