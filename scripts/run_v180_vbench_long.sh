...
*)
    echo "usage: bash scripts/run_v180_vbench_long.sh ACTION"
...
RUN_ROOT="${RUN_ROOT:-$ROOT/runs/v180_rccp_fresh128}"
...
if [[ "$ACTION" == "decision" ]]; then
    "$PYTHON_BIN" - "$ANALYSIS_ROOT/v180_fresh128_metrics.json" <<'PY'
import json
import sys
...
accepted = {
    "fresh128_quality_identity_motion_confirmed",
    "fresh128_quality_identity_confirmed",
    "fresh128_identity_motion_confirmed",
}
...
if [[ "$ACTION" == "prepare" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT/scripts/prepare_v180_vbench_comparison.py" \
...
if [[ "$ACTION" == "split" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT/scripts/prepare_v175_vbench_splits.py" \
        ...
        --workers "${V180_SPLIT_WORKERS:-2}" --node-rank "$NODE_RANK" \
        --num-nodes "$NUM_NODES"
    exit $?
fi
...
TORCH_HUB_DIR="${V180_TORCH_HUB_DIR:-$ROOT/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V180_RUNTIME_HOME:-$ROOT/runs/_model_cache/dreamsim_home}"
if [[ ( "$ACTION" == "eval" || "$ACTION" == "resume-missing" ) && \
      "${V180_LOCAL_MODELS:-1}" == "1" ]]; then
...
EXTRA_ARGS=()
if [[ "${V180_LOCAL_MODELS:-1}" == "1" ]]; then
    EXTRA_ARGS+=(--local-models --torch-hub-dir "$TORCH_HUB_DIR" --runtime-home "$RUNTIME_HOME")
fi

"$PYTHON_BIN" "$ROOT/scripts/run_v180_vbench_long.py" "$PYTHON_ACTION" \
    --comparison-root "$COMPARISON_ROOT" --vbench-root "$VBENCH_ROOT" \
    ...
    --summary-stem vbench_core9_summary --analysis-stem v180_vbench_analysis \
    --summary-title "v180 RCCP Fresh128 VBench-Long" "${EXTRA_ARGS[@]}"

if [[ "$ACTION" == "collect" ]]; then
    "$PYTHON_BIN" "$ROOT/scripts/analyze_v180_fresh128_metrics.py" \
        --comparison-root "$COMPARISON_ROOT" \
        --summary "$SUMMARY_ROOT/vbench_core9_summary.json" \
        --parts-root "$PARTS_ROOT" \
        --output "$ANALYSIS_ROOT/v180_fresh128_metrics.json"
fi
