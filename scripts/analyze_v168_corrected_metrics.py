#!/usr/bin/env python3
"""Analyze v168 paired VBench metrics and make a development decision."""
...
REFERENCES = (MULTISCALE_MOTION, SF, STATE_RANK_MOTION)


def parse_args() -> argparse.Namespace:
    root = Path(__file__).resolve().parents[1]
    run_root = root / "runs" / "v168_cross_scale_consensus_moviebench16" / "full8"
    ...
    parser.add_argument(
        ...
        default=(
            run_root
            / "automated_screen"
            / "cross_scale_consensus_trace.json"
        ),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=run_root / "analysis" / "v168_corrected_metrics.json",
    )
...
def paired_reports(rows: dict) -> dict:
    ...
    for candidate_index, candidate in enumerate(CANDIDATES):
        reports[candidate] = {}
        references = tuple(
            method for method in METHODS if method != candidate
        )
...
def development_decision(
    *,
    ...
    for candidate in CANDIDATES:
        ...
        metric_gate = all(nonnegative_frontier.values())
        decisions[candidate] = {
            ...
            "nonnegative_frontier_checks": nonnegative_frontier,
            "metric_gate": metric_gate,
            "eligible_for_128_prompt_confirmation": (
                mechanism_gate and metric_gate
            ),
        }
    eligible = [
        candidate
    ...
    review_candidate = selected or max(
        CANDIDATES,
    ...
    return {
        ...
        "recommendation": (
            ...
            else "do_not_scale; inspect_the_automatic_failure_cases"
        ),
        "rule": (
            "mechanism gate plus nonnegative aggregate deltas in official "
            "Quality, identity/background, temporal mechanics, and dynamic "
            "degree relative to frozen v166 MultiScaleMotion"
        ),
        "claim_boundary": (
...
def write_markdown(path: Path, report: dict) -> None:
    ...
    lines.extend(
        [
            "",
            "The decision uses mutually exclusive diagnostic groups and the "
            "official VBench Quality Score. Human review is capped at two "
            "automatically selected prompts.",
            "",
        ]
    )
...
def main() -> None:
    ...
    if mechanism.get("experiment") != "v168_cross_scale_consensus_trace":
        raise ValueError("not a v168 mechanism trace report")
