...
RUN_NAME="v171_demand_gated_motion_moviebench16"
SMOKE_PROMPT_INDEX="${V171_SMOKE_PROMPT_INDEX:-6}"
...
export V171_REUSE_V170_ROOT="${V171_REUSE_V170_ROOT:-${ROOT}/runs/v170_matched_attribution_moviebench16/full8}"
export V120_BASELINE_ONLY=0
export V120_OURS_ONLY=1
export V119_PROMOTION_APPROVED=1
export PYRAMIDKV_POLICY_TRACE_HEADS=0

CANDIDATES="middle10_reservoir2_multiscalemotion1,middle10_reservoir2_deficitquery1,middle10_reservoir2_deficitbaseline1"
runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v171_demand_gated_motion_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
...
case "${ACTION}" in
  offline)
    ...
    "${PYTHON_BIN}" \
      ...
      --trace-dir "${V171_REUSE_V170_ROOT}/traces" \
      --output "${ROOT}/runs/${RUN_NAME}/offline/v171_counterfactual.json"
    ;;
  smoke)
    ...
    V171_SMOKE_PROMPT_INDEX="${SMOKE_PROMPT_INDEX}" \
      "${runner[@]}" generate
    ;;
  preflight|generate)
    [[ "${NUM_NODES}" -ge 2 ]] || {
  ...
  mechanism)
    ...
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/analyze_v171_demand_gated_trace.py" \
  ...
  status)
    ...
    ;;
  package)
    package_path="${OUT_ROOT}/v171_diagnostics.tar.gz"
