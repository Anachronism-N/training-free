...
RUN_NAME="v166_multiscale_motion_moviebench16"
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/${RUN_NAME}/full8}"
NODE_RANK="${NODE_RANK:-0}"
NUM_NODES="${NUM_NODES:-4}"
GPU_LIST="${GPU_LIST:-0,1,2,3,4,5,6,7}"

DEFAULT_REUSE_ROOT="${ROOT}/runs/v165_direction_stale_tie_moviebench16/full8"
if [[ -z "${V166_REUSE_V165_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V166_REUSE_V165_ROOT="${DEFAULT_REUSE_ROOT}"
fi
export V120_BASELINE_ONLY=0
export V120_OURS_ONLY=0

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v166_multiscale_motion_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
  ...
  --num-nodes "${NUM_NODES}"
  --gpu-list "${GPU_LIST}"
  --candidates "middle10_reservoir2_directionmatch1,middle10_reservoir2_dirstaletie005,middle10_reservoir2_multiscaledir1,middle10_reservoir2_multiscalemotion1,middle10_reservoir2_statemotionpair1_reference"
)
...
case "${ACTION}" in
  ...
  mechanism)
    "${PYTHON_BIN}" \
      "${ROOT}/scripts/analyze_v166_multiscale_motion_trace.py" \
      --trace-dir "${OUT_ROOT}/traces" \
      --output "${OUT_ROOT}/automated_screen/multiscale_motion_trace.json"
    ;;
  status)
    ...
    ;;
  package)
    package_path="${OUT_ROOT}/v166_diagnostics.tar.gz"
