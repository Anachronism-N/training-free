#!/usr/bin/env python3
"""Refine retrieval and sink allocation under the frozen old-v98 304/56 map."""
...
EXPERIMENT = "v119_candidate_refinement_1video"

# Existing v116 controls are intentionally not regenerated:
# landmark_motion1, landmark_retrieval2, and prototype_motion1.
CELLS = (
    Cell(
...
RETIRED_SINK3_CELLS = (
    Cell(
        ...
        history_budget_profile="sink3_extra",
        max_full_frame_equivalents=11,
    ),
    Cell(
        ...
        support_policy="landmark",
        suppress_policy="motion_pair1",
        history_budget_profile="sink3_budget9",
    ),
)
...
def validate_inputs(args: argparse.Namespace) -> list[str]:
    ...
    if args.seed != 0:
        raise SystemExit("the frozen v119 screen requires seed 0")
    required = (
...
def main() -> None:
    ...
    contract = {
        ...
        "cache_allocations": {
            "retrieval1": {
                "supportive": "sink1 + landmark4 + recent4 = 9",
                "suppressive": "sink1 + retrieval1 + recent7 = 9",
            },
            "retrieval1_age24": {
                "supportive": "sink1 + landmark4 + recent4 = 9",
                "suppressive": (
                    "sink1 + retrieval1(age<=24 latent frames) + recent7 = 9"
                ),
            },
            "retrieval_motion": {
                "supportive": "sink1 + landmark4 + recent4 = 9",
                "suppressive": (
                    "sink1 + retrieval1(age<=24) + one adjacent "
        ...
        "invariants": {
            ...
            "retrieval_archive_bounded": True,
            "retrieval_age_is_audited": True,
        },
        "reused_controls": [
    ...
    print(
        f"[v119] mode={args.mode} node={args.node_rank}/{args.num_nodes} "
