#!/usr/bin/env bash
# Evaluate one promoted v112 matrix with VBench-Long after human review.
set -uo pipefail

ACTION="${1:-}"
CANDIDATE="${2:-}"
if [[ "$ACTION" != "eval" && "$ACTION" != "collect" ]]; then
    echo "usage: bash scripts/run_v112_vbench_long.sh eval|collect CANDIDATE"
    exit 2
fi
if [[ -z "$CANDIDATE" ]]; then
...
if [[ "${V111_PROMOTION_APPROVED:-0}" != "1" ]]; then
    echo "[blocked] set V111_PROMOTION_APPROVED=1 after the v111 review"
    exit 2
fi

ROOT="${REPO_ROOT:-/apdcephfs_gy2/share_303214315/cedricnie/develop/training-free}"
RUN_ROOT="${RUN_ROOT:-$ROOT/runs/v112_role_event_cache_32prompt/$CANDIDATE}"
...
[[ -s "$MANIFEST" ]] || {
    echo "[error] missing audited v112 manifest: $MANIFEST"
    exit 2
}
mapfile -t METHODS < <(
    python - "$MANIFEST" <<'PY'
