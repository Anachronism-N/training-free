...
RUN_NAME="v163_statemotion_strict_moviebench16"
...
runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v163_statemotion_strict_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
  ...
  --num-nodes "${NUM_NODES}"
  --gpu-list "${GPU_LIST}"
  --candidates "middle10_reservoir2_statemotion1_strict,middle10_reservoir2_freshmotionpair1_reference,middle10_reservoir2_motionpair1_reference,middle10_reservoir4_reference,all_recent8_reference"
)
...
case "${ACTION}" in
  preflight|generate|audit)
    ...
    ;;
  mechanism)
    "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v163_state_motion_trace.py" \
