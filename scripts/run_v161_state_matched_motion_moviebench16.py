#!/usr/bin/env python3
"""Run the v161 state-matched coherent-motion retrieval experiment."""
...
import run_v160_fresh_motion_moviebench16 as v160
...
EXPERIMENT = "v161_state_matched_motion_moviebench16"
PROMPT_COUNT = 16
PRIMARY = "ours_middle10_reservoir2_statemotionpair1"
FRESH_REFERENCE = "ours_middle10_reservoir2_freshmotionpair1_reference"
...
REUSE_METHODS = {
    "sf_native": "sf_native",
    FRESH_REFERENCE: "ours_middle10_reservoir2_freshmotionpair1",
    "ours_middle10_reservoir2_motionpair1_reference": (
        "ours_middle10_reservoir2_motionpair1_reference"
    ),
    "ours_middle10_reservoir4_reference": (
...
V161_CELLS = (
    Cell(
        "v161_middle10_reservoir2_statemotionpair1",
        "v161_state_motion",
        "single",
    ...
    Cell(
        "v161_middle10_reservoir2_freshmotionpair1_reference",
        "v161_reference",
        "single",
    ...
    Cell(
        "v161_middle10_reservoir2_motionpair1_reference",
        "v161_reference",
        "single",
    ...
    Cell(
        "v161_middle10_reservoir4_reference",
        "v161_reference",
        "single",
...
def configure_parent_runner() -> None:
    ...
    runner.PUBLISHED_TAG = "v161"
    runner.RUN_LABEL = "v161"
    ...
    runner.DEFAULT_CANDIDATES = (
        "middle10_reservoir2_statemotionpair1",
    ...
    runner._CELLS_BY_NAME.update({cell.name: cell for cell in V161_CELLS})
    runner._CANDIDATE_SPECS.update(
        {
            "middle10_reservoir2_statemotionpair1": (
                "v161_middle10_reservoir2_statemotionpair1",
                "state_motion_primary",
            ),
            "middle10_reservoir2_freshmotionpair1_reference": (
                "v161_middle10_reservoir2_freshmotionpair1_reference",
...
def v160_run_root() -> Path:
    default = ROOT / "runs" / v160.EXPERIMENT / "full8"
    return Path(os.environ.get("V161_REUSE_V160_ROOT", default)).resolve()


def _v160_result_path(name: str) -> Path:
    return ROOT / "docs" / "results" / v160.EXPERIMENT / name


def load_v160_source(prompt_manifest: dict, *, pf_runtime: dict) -> dict:
    root = v160_run_root()
    published_path = root / "published_manifest.json"
    contract_path = root / "contracts" / "experiment.json"
    trace_path = _v160_result_path("fresh_motion_trace.json")
    screen_path = _v160_result_path("automated_screen.json")
    human_path = _v160_result_path("adaptive_review_analysis.json")
    for path in (
        published_path,
        contract_path,
        trace_path,
        ...
        if not path.is_file():
            raise ValueError(f"missing frozen v160 source: {path}")
    ...
    if (
        ...
        or published.get("experiment") != v160.EXPERIMENT
        ...
        or screen.get("automatic_safety_screen") is not True
        or int(wave1.get("primary_severe_count", -1)) < 1
        or human.get("wave1_decision") != "continue_wave2"
    ):
        raise ValueError("v160 source does not match the frozen v161 rationale")
    ...
    for target, source_method in REUSE_METHODS.items():
        ...
        if {path.name for path in video_dir.glob("*.mp4")} != expected_names:
            raise ValueError(f"incomplete v160 reuse source: {source_method}")
        sources[target] = {
    ...
    return {
        ...
        "fresh_motion_trace": str(trace_path),
        "fresh_motion_trace_sha256": sha256(trace_path),
        "automated_screen": str(screen_path),
        "automated_screen_sha256": sha256(screen_path),
        "wave1_human_analysis": str(human_path),
        "wave1_human_analysis_sha256": sha256(human_path),
...
def run_reused_task(
    ...
    *,
    ...
    reuse = args.v161_source["sources"][method.key]
    ...
    write_frozen(
        marker,
        {
            ...
            "reuse_manifest_sha256": args.v161_source[
    ...
    contract.update(
        {
            ...
            "v160_source": args.v161_source,
            "cache_contract": {
                "selected_layers": (
                    "sink1 + reservoir2 + state-matched motion pair1 + recent4"
                ),
                ...
                "abstention": (
                    "read no motion pair when no full adjacent pair passes "
                    "age, state, and non-opposite-direction gates"
                ),
            ...
            "design": {
                ...
                "isolated_change_from_v160": (
                    "replace forced single-pair read with bounded multi-pair "
                    "archive and online state/direction-compatible readout"
                ),
                "fixed_factors": (
            ...
            "evaluation": {
                "mechanism_gate": (
                    "archive has multiple eligible pairs and state matching "
                    "changes at least one read or abstains on incompatibility"
                ),
            ...
            "claim_boundary": (
                "v161 is a failure-focused exploratory recovery experiment. "
...
def main() -> None:
    ...
    args.v161_source = load_v160_source(
        prompt_manifest,
    ...
    if tuple(method.key for method in methods) != EXPECTED_METHOD_KEYS:
        raise SystemExit(
            "v161 requires the frozen six-method order: "
    ...
    if args.mode == "preflight":
        ...
        print(
            f"[v161-preflight] PASS node={args.node_rank}/{args.num_nodes} "
    ...
    if args.mode == "audit":
        ...
        print(
            f"[v161-audit] PASS methods={len(payload['methods'])} "
    ...
    if not summary["ok"]:
        raise SystemExit("\n".join(failures or ["v161 task count mismatch"]))
