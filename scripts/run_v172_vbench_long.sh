...
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/v172_relative_depth_moviebench16/full8}"
...
DECISION_JSON="${DECISION_JSON:-${ANALYSIS_ROOT}/v172_depth_metrics.json}"
...
run_decision() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v172_depth_metrics.py" \
...
if [[ "${ACTION}" == "prepare" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v172_vbench_comparison.py" \
    --run-root "${RUN_ROOT}" \
...
if [[ "${ACTION}" == "split" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v172_vbench_splits.py" \
    --comparison-root "${COMPARISON_ROOT}" \
    --vbench-root "${VBENCH_ROOT}" \
    --workers "${V172_SPLIT_WORKERS:-2}" \
    --node-rank "${NODE_RANK}" \
    --num-nodes "${NUM_NODES}"
  exit $?
fi
...
TORCH_HUB_DIR="${V172_TORCH_HUB_DIR:-${ROOT}/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V172_RUNTIME_HOME:-${ROOT}/runs/_model_cache/dreamsim_home}"
if [[ ( "${ACTION}" == "eval" || "${ACTION}" == "resume-missing" ) && \
      "${V172_LOCAL_MODELS:-1}" == "1" ]]; then
...
"${PYTHON_BIN}" "${ROOT}/scripts/run_v172_vbench_long.py" "${PYTHON_ACTION}" \
  ...
  --analysis-stem "v172_vbench_analysis" \
  --summary-title "v172 Relative-Depth VBench-Long Core-9" \
  "${EXTRA_ARGS[@]}"
