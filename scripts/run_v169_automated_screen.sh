...
run_mechanism() {
  "${PYTHON_BIN}" \
    ...
    --output "${METRICS}/soft_cross_scale_trace.json"
}
...
run_screen() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v169_automated_screen.py" \
...
case "${ACTION}" in
  ...
  all)
    ...
    run_comprehensive
    run_screen
    echo "automatic screen complete; do not review unless VBench requests it"
    ;;
  *)
