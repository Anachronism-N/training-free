#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for v169."""
...
def analyze(payload: dict) -> dict:
    ...
    if tuple(rows) != METHODS or dimensions != DIMENSIONS or payload.get("missing"):
        raise ValueError("v169 VBench summary violates the frozen grid")
    for method in METHODS:
        row = rows[method]
        if set(row) != set(DIMENSIONS):
            raise ValueError(f"{method}: incomplete v169 VBench dimensions")
        if (
            abs(float(row["overall_consistency"]) - float(row["temporal_style"]))
            > 1e-12
        ):
            raise ValueError(
    ...
    comparisons = {
        candidate: {
            reference: {
                "exclusive_delta": {
                    metric: derived[candidate][metric] - derived[reference][metric]
                    for metric in EXCLUSIVE_GROUPS
                },
                "official_quality_delta": quality[candidate] - quality[reference],
            }
    ...
    return {
        ...
        "primary_reference": MULTISCALE_MOTION,
        "candidate_comparisons": comparisons,
        "metric_promotion_gate": False,
        "claim_boundary": (
            "Aggregate collection does not promote a method. Run paired "
            "corrected analysis and the mechanism audit; these 16 prompts "
            "remain adaptive development evidence."
        ),
    }
...
def configure_base() -> None:
    ...
    base.ANALYSIS_STEM = "v169_vbench_analysis"
    base.SUMMARY_TITLE = "v169 Soft Cross-scale VBench-Long Core-9"
