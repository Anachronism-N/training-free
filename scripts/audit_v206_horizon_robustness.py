#!/usr/bin/env python3
"""Audit one v206 robustness scope and publish complete artifacts only."""
    ...
    *,
    ...
    if smoke_prompt_index is None:
        ...
        if not 0 <= smoke_prompt_index < prompt_count:
            raise ValueError("v206 smoke prompt index is outside the frozen scope")
    ...
    for method in methods:
        ...
        if method_ok:
            ...
            if unexpected:
                raise RuntimeError(
                    f"refusing stale v206 published videos for {method}: "
                    f"{sorted(unexpected)}"
                )
            ...
            if {path.name for path in published_dir.glob("*.mp4")} != expected_published:
                raise RuntimeError(f"incomplete v206 published set for {method}")
        method_rows.append(
            {
                "key": method,
                "role": config["role"],
    ...
    contract = {
        "version": 1,
        "experiment": "v206_horizon_robustness_generation",
        "scope": scope_key,
    ...
    summary = {
        ...
        "link_counts": links,
        "audit_cost_note": (
            "All media and logs plus one complete route trace per cache candidate "
            "are checked; redundant per-shard route traces are intentionally disabled."
        ),
    }
    write_json(run_root / "audits" / "summary.json", summary)
    if not all_ok:
        failed = [row["key"] for row in method_rows if not row["ok"]]
        raise RuntimeError(f"v206 audit failed: {failed}")
