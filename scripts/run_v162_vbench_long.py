#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for v162 calibration."""
...
def analyze(payload: dict) -> dict:
    ...
    if (
        ...
        raise ValueError("v162 VBench summary violates the frozen grid")
    derived = {}
    for method in METHODS:
        row = rows[method]
        if set(row) != set(DIMENSIONS):
            raise ValueError(f"{method}: incomplete v162 VBench dimensions")
        derived[method] = {
    ...
    return {
        "version": 1,
        "experiment": "v162_automatic_calibration_vbench16",
        "methods": list(METHODS),
        "dimensions": list(DIMENSIONS),
        "derived_scores": derived,
        "claim_boundary": (
            "VBench is an automatic diagnostic input; human-prediction "
            "validity is decided separately by held-out calibration"
        ),
    }
...
def render_markdown(report: dict) -> str:
    ...
    lines.extend(
        [
            "",
            "These values do not replace the v162 metric-human calibration.",
            "",
        ]
    )
    return "\n".join(lines)
...
def configure_base() -> None:
    base.RUN_LABEL = "v162"
    base.SUMMARY_EXPERIMENT = "v162_automatic_calibration_vbench16"
    base.ANALYSIS_STEM = "v162_vbench_analysis"
    base.SUMMARY_TITLE = "v162 Automatic Calibration VBench-Long Core-9"
