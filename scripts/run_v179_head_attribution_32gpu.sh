...
OUT_ROOT="${V179_OUT_ROOT:-$ROOT/runs/v179_rccp_head_attribution}"
...
preflight() {
    activate_env
    for path in "$PF" "$CONFIG" "$CHECKPOINT" "$INPUT_MANIFEST" "$PROMPTS"; do
        [[ -e "$path" ]] || { echo "[error] missing $path"; exit 2; }
    done
    python "$ROOT/scripts/prepare_v179_head_attribution.py" verify \
...
run_shard() {
    ...
    if [[ "$FORCE" != "1" && -s "$marker" ]] && shard_complete "$raw_dir" "$rank"; then
        echo "[v179-skip] method=$method rank=$rank"
        return
    fi
...
generate32() {
    ...
    for method in "${methods[@]}"; do
        ...
        [[ "$failed" -eq 0 ]] || {
            echo "[error] v179 method=$method failed on node=$NODE_RANK"
            exit 1
        }
    done
}
...
package() {
    ...
    tar -C "$OUT_ROOT" -czf "$OUT_ROOT/v179_head_attribution_diagnostics.tar.gz" \
        inputs contracts audits published_manifest.json logs
    echo "$OUT_ROOT/v179_head_attribution_diagnostics.tar.gz"
}
