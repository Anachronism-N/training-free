...
run 4 20260720_v49b_no_routing no_routing "${COMMON[@]}" MEMORY_HEAD_ROUTING=static MEMORY_HEAD_LABELS="" &
# R1: PF static labels
run 5 20260720_v49b_pf_static pf_static "${COMMON[@]}" MEMORY_HEAD_ROUTING=static MEMORY_HEAD_LABELS=1,2 &
# R2: history confidence only
run 6 20260720_v49b_confidence confidence "${COMMON[@]}" MEMORY_HEAD_ROUTING=confidence_adaptive MEMORY_ROUTING_SHARPNESS=5.0 &
# R3: functional adaptive: confidence * margin * query stability
run 7 20260720_v49b_functional functional "${COMMON[@]}" MEMORY_HEAD_ROUTING=functional_adaptive MEMORY_ROUTING_SHARPNESS=5.0 MEMORY_MARGIN_THRESHOLD=0.10 MEMORY_QUERY_EMA_DECAY=0.90 &
