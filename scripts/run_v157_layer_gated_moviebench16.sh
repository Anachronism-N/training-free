...
OUT_ROOT="${OUT_ROOT:-${ROOT}/runs/v157_layer_gated_moviebench16/full8}"
NODE_RANK="${NODE_RANK:-0}"
NUM_NODES="${NUM_NODES:-4}"
GPU_LIST="${GPU_LIST:-0,1,2,3,4,5,6,7}"

DEFAULT_REUSE_ROOT="${ROOT}/runs/v155_profile_aligned_moviebench16/full7"
if [[ -z "${V157_REUSE_V155_ROOT:-}" && \
      -s "${DEFAULT_REUSE_ROOT}/published_manifest.json" ]]; then
  export V157_REUSE_V155_ROOT="${DEFAULT_REUSE_ROOT}"
fi

runner=(
  "${PYTHON_BIN}" "${ROOT}/scripts/run_v157_layer_gated_moviebench16.py"
  --repo-root "${ROOT}"
  --sf-repo "${SF_REPO}"
  --sf-config "${SF_CONFIG}"
...
case "${ACTION}" in
  ...
  status)
    ...
    ;;
  blind)
    "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v157_blind_review.py" \
      --run-root "${OUT_ROOT}"
    ;;
  package)
    package_path="${OUT_ROOT}/v157_diagnostics.tar.gz"
