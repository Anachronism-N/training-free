#!/usr/bin/env python3
"""Run prompt-correct VBench-Long core-9 for v168."""
...
def analyze(payload: dict) -> dict:
    ...
    if tuple(rows) != METHODS or dimensions != DIMENSIONS or payload.get("missing"):
        raise ValueError("v168 VBench summary violates the frozen grid")
    for method in METHODS:
        row = rows[method]
        if set(row) != set(DIMENSIONS):
            raise ValueError(f"{method}: incomplete v168 VBench dimensions")
        if abs(
    ...
    candidate_comparisons = {
        candidate: {
            reference: {
                "exclusive_delta": {
                    metric: derived[candidate][metric]
                    - derived[reference][metric]
                    for metric in EXCLUSIVE_GROUPS
                },
                "official_quality_delta": quality[candidate]
                - quality[reference],
            }
...
def configure_base() -> None:
    ...
    base.ANALYSIS_STEM = "v168_vbench_analysis"
    base.SUMMARY_TITLE = "v168 Cross-scale Consensus VBench-Long Core-9"
