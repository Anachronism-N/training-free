#!/usr/bin/env bash
# Deterministic Coverage operators under the schedule selected by v184.
set -euo pipefail
...
case "$ACTION" in
    ...
    *)
        echo "usage: bash scripts/run_v186_phase_operator_screen_32gpu.sh ACTION"
...
SOURCE_PROMPTS="${V186_SOURCE_PROMPTS:-/apdcephfs_gy2/share_303214315/cedricnie/develop/research_sprint/Causal-Forcing/prompts/MovieGen_128_qwen.txt}"
V184_ROOT="${V184_OUT_ROOT:-$ROOT/runs/v184_denoise_phase_coverage}"
V184_DECISION="${V184_DECISION:-$V184_ROOT/screen32/analysis/v184_denoise_phase_screen.json}"
V184_PUBLISHED="${V184_PUBLISHED:-$V184_ROOT/screen32/published_manifest.json}"
...
ALL_METHODS="phase_landmark,phase_prototype,phase_retrieval"
...
FRAMES="${FRAMES:-120}"
SEED="${SEED:-0}"
SMOKE_PROMPT_INDEX="${V186_SMOKE_PROMPT_INDEX:-3}"
FORCE="${FORCE:-0}"
...
[[ "$FRAMES" -eq 120 && "$SEED" -eq 0 ]] || {
    echo "[error] v186 is frozen at 120 latent frames and seed 0"
    exit 2
}
[[ "$SMOKE_PROMPT_INDEX" -ge 0 && "$SMOKE_PROMPT_INDEX" -lt 32 ]] || {
    echo "[error] V186_SMOKE_PROMPT_INDEX must be within [0, 31]"
    exit 2
}
...
for method in "${REQUESTED_METHODS[@]}"; do
    case ",$ALL_METHODS," in
        *",$method,"*) ;;
        *) echo "[error] unsupported v186 method: $method"; exit 2 ;;
    esac
    [[ -z "${METHOD_SEEN[$method]:-}" ]] || {
        echo "[error] duplicate v186 method: $method"
        exit 2
    }
...
prepare() {
    ...
    activate_env
    python "$ROOT/scripts/prepare_v186_phase_operator_screen.py" prepare \
        --source-prompts "$SOURCE_PROMPTS" \
        --v184-decision "$V184_DECISION" \
        --v184-published "$V184_PUBLISHED" \
...
preflight() {
    ...
    python "$ROOT/scripts/prepare_v186_phase_operator_screen.py" verify \
        --manifest "$MANIFEST"
...
configure_runtime() {
    ...
    unset PYRAMIDKV_CACHE_COMPAT_COVERAGE_OPERATOR
}
...
run_shard() {
    ...
    if [[ "$FORCE" != "1" && -s "$marker" ]] && \
       shard_complete "$raw_dir" "$prompt_count" "$rank" "$stride"; then
        echo "[v186-skip] scope=$(basename "$scope_root") method=$method rank=$rank"
        return
    fi
...
smoke() {
    ...
    [[ "$failed" -eq 0 ]] || { echo "[error] v186 smoke failed"; exit 1; }
}
...
generate32() {
    ...
    for method in "${REQUESTED_METHODS[@]}"; do
        echo "[v186-method] method=$method node=$NODE_RANK"
        local -a pids=()
        for slot in "${!GPUS[@]}"; do
        ...
        for pid in "${pids[@]}"; do wait "$pid" || failed=1; done
        [[ "$failed" -eq 0 ]] || {
            echo "[error] v186 method=$method failed on node=$NODE_RANK"
            exit 1
        }
    done
}
...
audit_scope() {
    ...
    preflight
    python "$ROOT/scripts/audit_v186_phase_operator_screen.py" \
...
package() {
    ...
    local target="$OUT_BASE/v186_phase_operator_diagnostics.tar.gz"
    tar -C "$OUT_BASE" -czf "$target" \
        ...
        screen32/metrics screen32/analysis screen32/logs screen32/traces
    echo "$target"
}
