...
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/v162_freshmotion4_moviebench16/full8}"
...
METHODS=(
  sf_native
  ours_middle10_reservoir2_freshmotion4
...
run_screen() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v162_automated_screen.py" \
...
prepare_wave() {
  local wave="$1"
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v162_adaptive_review.py" \
    --wave "${wave}" \
    --run-root "${RUN_ROOT}" \
...
case "${ACTION}" in
  ...
  all)
    ...
    if mechanism_passes; then
      ...
      echo "v162 mechanism gate failed; adaptive review was not prepared" >&2
    fi
    ;;
  review-wave1|review-wave2)
    if ! mechanism_passes; then
      echo "v162 mechanism gate must pass before human review" >&2
      exit 3
    fi
    prepare_wave "${ACTION##*wave}"
    ;;
  analyze-wave1)
    "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v162_adaptive_review.py" \
