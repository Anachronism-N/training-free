#!/usr/bin/env bash
# Classifier-holdout causal screen for frozen Head x Phase maps.
set -euo pipefail
...
case "$ACTION" in
    ...
    *)
        echo "usage: bash scripts/run_v190_head_phase_causal_screen_32gpu.sh ACTION"
...
OUT_BASE="${V190_OUT_ROOT:-$ROOT/runs/v190_head_phase_causal_screen}"
...
FRAMES="${FRAMES:-120}"
SEED="${SEED:-19000}"
SMOKE_PROMPT_INDEX="${V190_SMOKE_PROMPT_INDEX:-3}"
FORCE="${FORCE:-0}"
...
[[ "$FRAMES" -eq 120 && "$SEED" -eq 19000 ]] || {
    echo "[error] v190 is frozen at 120 latent frames and seed 19000"; exit 2;
}
[[ "$SMOKE_PROMPT_INDEX" -ge 0 && "$SMOKE_PROMPT_INDEX" -lt 32 ]] || {
    echo "[error] V190_SMOKE_PROMPT_INDEX must be within [0,31]"; exit 2;
}
...
prepare() {
    ...
    activate_env
    python "$ROOT/scripts/prepare_v190_head_phase_causal_screen.py" prepare \
        --v189-manifest "$V189_MANIFEST" --v189-analysis "$V189_ANALYSIS" \
...
preflight() {
    ...
    python "$ROOT/scripts/prepare_v190_head_phase_causal_screen.py" verify \
        --manifest "$MANIFEST"
    mapfile -t available < <(manifest_methods)
    mapfile -t requested < <(requested_methods)
    [[ "${#requested[@]}" -gt 0 ]] || { echo "[error] no v190 methods"; exit 2; }
    declare -A seen=()
    local method
    for method in "${requested[@]}"; do
        [[ " ${available[*]} " == *" $method "* ]] || {
            echo "[error] unsupported v190 method: $method"; exit 2;
        }
        [[ -z "${seen[$method]:-}" ]] || {
            echo "[error] duplicate v190 method: $method"; exit 2;
        }
        seen[$method]=1
    done
...
run_shard() {
    ...
    if [[ "$FORCE" != "1" && -s "$marker" ]] && \
       shard_complete "$raw_dir" "$prompt_count" "$rank" "$stride"; then
        echo "[v190-skip] scope=$(basename "$scope_root") method=$method rank=$rank"
        return
    fi
    ...
    if [[ "$smoke_mode" == "1" ]]; then
        ...
        while [[ "$cursor" -lt "$method_count" ]]; do
            ...
            for slot in "${!GPUS[@]}"; do
                ...
                run_shard "$scope_root" "$method" 32 "$start" 32 "${GPUS[$slot]}" &
                pids+=("$!")
            done
            failed=0
            for pid in "${pids[@]}"; do wait "$pid" || failed=1; done
            [[ "$failed" -eq 0 ]] || { echo "[error] v190 smoke wave failed"; exit 1; }
            cursor=$((cursor + GPUS_PER_NODE))
        done
        return
    fi
    for method in "${ordered[@]}"; do
        echo "[v190-method] method=$method node=$NODE_RANK"
        local -a pids=()
        for slot in "${!GPUS[@]}"; do
        ...
        for pid in "${pids[@]}"; do wait "$pid" || failed=1; done
        [[ "$failed" -eq 0 ]] || {
            echo "[error] v190 method=$method failed on node=$NODE_RANK"; exit 1;
        }
    done
}
...
audit_scope() {
    ...
    preflight
    python "$ROOT/scripts/audit_v190_head_phase_causal_screen.py" \
...
package() {
    ...
    local target="$OUT_BASE/v190_head_phase_causal_screen_diagnostics.tar.gz"
