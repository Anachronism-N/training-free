...
RUN_ROOT="${RUN_ROOT:-${ROOT}/runs/v169_soft_cross_scale_moviebench16/full8}"
...
MECHANISM_TRACE="${MECHANISM_TRACE:-${RUN_ROOT}/automated_screen/soft_cross_scale_trace.json}"
DECISION_JSON="${DECISION_JSON:-${ANALYSIS_ROOT}/v169_corrected_metrics.json}"
...
run_decision() {
  "${PYTHON_BIN}" "${ROOT}/scripts/analyze_v169_corrected_metrics.py" \
...
if [[ "${ACTION}" == "prepare-review" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v169_minimal_review.py" \
...
if [[ "${ACTION}" == "prepare" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v169_vbench_comparison.py" \
    --run-root "${RUN_ROOT}" \
...
if [[ "${ACTION}" == "split" ]]; then
  ...
  "${PYTHON_BIN}" "${ROOT}/scripts/prepare_v169_vbench_splits.py" \
    --comparison-root "${COMPARISON_ROOT}" \
    --vbench-root "${VBENCH_ROOT}" \
    --workers "${V169_SPLIT_WORKERS:-2}" \
    --node-rank "${NODE_RANK}" \
    --num-nodes "${NUM_NODES}"
  exit $?
fi
...
TORCH_HUB_DIR="${V169_TORCH_HUB_DIR:-${ROOT}/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V169_RUNTIME_HOME:-${ROOT}/runs/_model_cache/dreamsim_home}"
if [[ ( "${ACTION}" == "eval" || "${ACTION}" == "resume-missing" ) && \
      "${V169_LOCAL_MODELS:-1}" == "1" ]]; then
...
"${PYTHON_BIN}" "${ROOT}/scripts/run_v169_vbench_long.py" "${PYTHON_ACTION}" \
  ...
  --analysis-stem "v169_vbench_analysis" \
  --summary-title "v169 Soft Cross-scale VBench-Long Core-9" \
  "${EXTRA_ARGS[@]}"
