...
case "$ACTION" in
    ...
    *)
        echo "usage: bash scripts/run_v184_vbench_long.sh {prepare|split|preflight|eval|resume-missing|status|collect|decision}"
        exit 2
        ;;
esac

ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
RUN_ROOT="${RUN_ROOT:-$ROOT/runs/v184_denoise_phase_coverage/screen32}"
...
if [[ "$ACTION" == "prepare" ]]; then
    ...
    "$PYTHON_BIN" "$ROOT/scripts/prepare_v184_vbench_comparison.py" \
...
if [[ "$ACTION" == "decision" ]]; then
    ...
    "$PYTHON_BIN" - "$ANALYSIS_ROOT/v184_denoise_phase_screen.json" <<'PY'
import json
import sys
from pathlib import Path

payload = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
print(f"[v184-decision] {payload['recommendation']}")
print("promoted=" + ",".join(payload["promoted_to_operator_screen"]))
...
TORCH_HUB_DIR="${V184_TORCH_HUB_DIR:-$ROOT/runs/_model_cache/torch_hub}"
RUNTIME_HOME="${V184_RUNTIME_HOME:-$ROOT/runs/_model_cache/dreamsim_home}"
if [[ ( "$ACTION" == "eval" || "$ACTION" == "resume-missing" ) && \
      "${V184_LOCAL_MODELS:-1}" == "1" ]]; then
...
"$PYTHON_BIN" "$ROOT/scripts/run_v174_vbench_long.py" "$PYTHON_ACTION" \
    ...
    --analysis-stem v184_vbench_analysis \
    --summary-title "v184 Denoising-Phase Coverage VBench-Long (development32)" \
    "${EXTRA_ARGS[@]}"

if [[ "$ACTION" == "collect" ]]; then
    "$PYTHON_BIN" "$ROOT/scripts/analyze_v184_denoise_phase_screen.py" \
        --comparison-root "$COMPARISON_ROOT" \
        --summary "$SUMMARY_ROOT/vbench_core9_summary.json" \
        --parts-root "$PARTS_ROOT" \
        --output "$ANALYSIS_ROOT/v184_denoise_phase_screen.json"
fi
