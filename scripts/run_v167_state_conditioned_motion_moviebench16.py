#!/usr/bin/env python3
"""Run the v167 state-conditioned motion-recall experiment."""
...
PROMPT_COUNT = 16
DIRECTION_MATCH = v166.DIRECTION_MATCH
MULTISCALE_MOTION = v166.MULTISCALE_MOTION
STATE_REFERENCE = v166.STATE_REFERENCE
...
NEW_METHODS = {STATE_RANK_MOTION, DEFICIT_STATE_RANK_MOTION}
_PARENT_RUN_TASK = runner.run_task


V167_CELLS = (
    Cell(
        "v167_middle10_reservoir2_directionmatch1",
        "v167_reference",
        "single",
    ...
    Cell(
        "v167_middle10_reservoir2_multiscalemotion1",
        "v167_reference",
        "single",
    ...
    Cell(
        "v167_middle10_reservoir2_statemotionpair1_reference",
        "v167_reference",
        "single",
    ...
    Cell(
        "v167_middle10_reservoir2_staterankmotion1",
        "v167_state_conditioned_motion",
        "single",
    ...
    Cell(
        "v167_middle10_reservoir2_deficitstaterankmotion1",
        "v167_state_conditioned_motion",
        "single",
        support_policy="reservoir2_deficitstaterankmotion1",
        suppress_policy="recent8_sink1",
        map_key="middle10",
    ),
)
...
def text_sha256_lf(path: Path) -> str:
    """Hash tracked text without platform-specific newline differences."""
...
def configure_parent_runner() -> None:
    ...
    runner.PUBLISHED_TAG = "v167"
    runner.RUN_LABEL = "v167"
    ...
    runner.DEFAULT_CANDIDATES = (
        ...
        "middle10_reservoir2_deficitstaterankmotion1",
    )
    runner._CELLS_BY_NAME.update({cell.name: cell for cell in V167_CELLS})
    runner._CANDIDATE_SPECS.update(
        {
            ...
            "middle10_reservoir2_staterankmotion1": (
                "v167_middle10_reservoir2_staterankmotion1",
                "reference_residual_state_filter",
            ),
            "middle10_reservoir2_deficitstaterankmotion1": (
                "v167_middle10_reservoir2_deficitstaterankmotion1",
                "deficit_gated_state_conditioned_recall",
            ),
        }
    )
...
def maybe_filter_smoke_tasks(tasks):
    raw = os.environ.get("V167_SMOKE_PROMPT_INDEX")
    if raw is None or not raw.strip():
        return list(tasks)
    prompt_index = int(raw)
    if not 0 <= prompt_index < PROMPT_COUNT:
        raise ValueError("V167_SMOKE_PROMPT_INDEX must be within [0, 15]")
    selected = [
        task
        for task in tasks
...
def v166_run_root() -> Path:
    ...
    return Path(os.environ.get("V167_REUSE_V166_ROOT", default)).resolve()
...
def load_v166_source(prompt_manifest: dict, *, pf_runtime: dict) -> dict:
    ...
    trace_path = (
        root / "automated_screen" / "multiscale_motion_trace.json"
    )
    ...
    if (
        ...
        raise ValueError("v166 source does not match the frozen v167 contract")
    ...
    return {
        ...
        "multiscale_motion_trace": str(trace_path),
        "multiscale_motion_trace_sha256": text_sha256_lf(trace_path),
        "multiscale_motion_trace_raw_sha256": sha256(trace_path),
    ...
    contract.update(
        {
            ...
            "cache_contract": {
                ...
                "state_similarity_role": (
                    "reference-centered residual rank; retain top half of "
                    "direction-compatible candidates"
                ),
            ...
            "motion_signature": {
                ...
                "multiscale_direction": (
                    "arithmetic mean of local and context cosine similarity"
                ),
                ...
                "magnitude_match": (
                    "geometric mean of local and context magnitude ratios"
                ),
                "multiscale_direction_score": "multiscale_direction",
                "multiscale_motion_score": (
                    "multiscale_direction multiplied by magnitude_match"
                ),
                "ranking": "score descending, then newer pair",
                "tuned_parameters": [],
            },
            "state_conditioning": {
                "raw_state_problem": (
                    "absolute clean-KV cosine is concentrated near one"
                ),
                "residual": "normalize(z_t - z_reference)",
                "similarity": (
                    "cosine between current and candidate endpoint residuals"
                ),
                "filter": (
                    "retain ceil(N/2) candidates by residual-state rank, "
                    "then maximize the frozen multiscale-motion score"
                ),
                "absolute_threshold": None,
            },
            "motion_deficit_gate": {
                "baselines": (
                    "online medians of prior local and context-per-step "
                    "descriptor magnitudes"
                ),
                "warmup_blocks": 4,
                "trigger": "both current magnitudes are below their medians",
                "triggered_read": "state-conditioned motion recall",
                "healthy_read": "newest direction-compatible atomic pair",
                "tuned_thresholds": [],
            },
            "design": {
                "new_methods": [
                ...
                "isolated_test": (
                    "first add relative state conditioning to v166 motion "
                    "recall, then gate the same recall on online motion deficit"
                ),
                "fixed_factors": (
            ...
            "evaluation": {
                "mechanism_gate": (
                    "residual-state filtering must alter at least one v166 "
                    "motion selection; the deficit method must exercise both "
                    "healthy and deficit branches; all reads remain atomic"
                ),
                "automatic_first": (
                ...
                "promotion_requires": (
                    "improve the corrected quality-motion frontier over "
                    "DirectionMatch without the v166 background-drift failures"
                ),
            },
            "claim_boundary": (
                "v167 is a 16-prompt development experiment and cannot be "
...
def main() -> None:
    ...
    if tuple(method.key for method in methods) != EXPECTED_METHOD_KEYS:
        raise SystemExit(
            "v167 requires the frozen six-method order: "
    ...
    if args.mode == "audit":
        ...
        print(
            f"[v167-audit] PASS methods={len(payload['methods'])} "
    ...
    if not summary["ok"]:
        raise SystemExit("\n".join(failures or ["v167 task count mismatch"]))
