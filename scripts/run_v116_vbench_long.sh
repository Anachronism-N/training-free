#!/usr/bin/env bash
# Evaluate an audited v116 MovieBench-16 method set with VBench-Long.
set -uo pipefail

ACTION="${1:-}"
if [[ "$ACTION" != "eval" && "$ACTION" != "collect" ]]; then
    echo "usage: bash scripts/run_v116_vbench_long.sh eval|collect"
    exit 2
fi
...
if int(payload.get("prompt_count", 0)) != 16:
    raise SystemExit("published manifest is not MovieBench-16")
