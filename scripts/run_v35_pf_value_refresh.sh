...
MEMORY_POSITION_MODE="${MEMORY_POSITION_MODE:-none}"
MEMORY_PROMPT_PRIOR_WEIGHT="${MEMORY_PROMPT_PRIOR_WEIGHT:-0.0}"
...
PYTHONPATH="$ROOT/src${PYTHONPATH:+:$PYTHONPATH}" CUDA_VISIBLE_DEVICES="$GPU" python inference.py \
    ...
    --pyramidkv_structured_memory_position_mode "$MEMORY_POSITION_MODE" \
    --pyramidkv_structured_memory_prompt_prior_weight "$MEMORY_PROMPT_PRIOR_WEIGHT" \
    "${EXTRA_ARGS[@]}"
