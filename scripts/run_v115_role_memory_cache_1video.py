#!/usr/bin/env python3
"""Screen role-conditioned memory mechanisms under the old-v98 304/56 map."""
...
# All cells read exactly nine full-frame equivalents:
#   full middle: sink1 + middle4 + recent4
#   compact middle: sink1 + middle2 + recent6
#   sparse middle: sink1 + 4 * 0.75-frame snapshots + recent5
CELLS = (
    # Supportive route sweep; Suppressive stays at the safest local control.
    Cell(
    ...
    Cell(
        ...
        "support",
        "single",
        support_policy="sparse75",
        suppress_policy="recent8_sink1",
    ),
    # Suppressive route sweep; Supportive stays at the strongest v111 route.
    Cell(
        "legacy_v98_support_landmark4_suppress_prototype2",
        "suppress",
        "single",
        support_policy="landmark",
        suppress_policy="prototype2",
    ),
    Cell(
    ...
    Cell(
        ...
        "single",
        support_policy="landmark",
        suppress_policy="sparse75",
    ),
    # Joint candidates only after each component has an isolated comparison.
    Cell(
    ...
    # Same-route controls test whether the binary routing itself matters.
    Cell(
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "mode",
        nargs="?",
        choices=("support", "suppress", "joint", "controls", "all"),
        default="all",
    )
    parser.add_argument(
        "--node-rank",
...
def validate_inputs(args: argparse.Namespace) -> list[str]:
    ...
    if args.seed != 0:
        raise SystemExit("the frozen v115 screen requires seed 0")
    required = (
...
def main() -> None:
    ...
    contract = {
        ...
        "selection_rules": {
            "prototype": (
                "adjacent semantic/motion coherent runs are represented by "
                "one exact-frame medoid; no KV averaging"
            ),
            "snapshot": (
                "one coherent full-frame snapshot per block is scored by "
                "query relevance plus archive uniqueness"
            ),
            "retrieval": (
                "one candidate per block enters a bounded archive; MMR reads "
                "two or four non-recent exact frames"
            ),
            "sparse75": (
                "snapshot selection plus 75% role-shared tokens, split "
                "between uniform coverage and saliency"
            ),
            "motion_pair1": (
                "one semantically coherent high-motion adjacent pair plus "
                "six recent frames"
            ),
        },
        "reused_v111_controls": [
    ...
    print(
        f"[v115] mode={args.mode} node={args.node_rank}/{args.num_nodes} "
