#!/usr/bin/env python3
"""Materialize prompt-correct VBench-Long inputs for v207."""
...
def link_or_validate(source: Path, target: Path) -> str:
    ...
    if target.exists() or target.is_symlink():
        if not target.samefile(source):
            raise RuntimeError(f"refusing mixed v207 VBench input: {target}")
        return "existing"
    try:
...
def prepare(run_root: Path, comparison_root: Path) -> dict:
    ...
    if not published_path.is_file() or not contract_path.is_file():
        raise ValueError("v207 screen32 generation must be audited first")
    published = json.loads(published_path.read_text(encoding="utf-8"))
    contract = json.loads(contract_path.read_text(encoding="utf-8"))
    if (
        published.get("ok") is not True
        or published.get("experiment") != f"{SOURCE_EXPERIMENT}_generation"
        ...
        raise ValueError("invalid v207 generation artifacts")
    ...
    if set(rows) != set(METHOD_ORDER) or any(not rows[key]["ok"] for key in METHOD_ORDER):
        raise ValueError("v207 method membership or audit drift")
    ...
    if (
        ...
        raise ValueError("v207 prompt provenance drift")
    ...
    for method in METHOD_ORDER:
        ...
        if {path.name for path in source_dir.glob("*.mp4")} != expected:
            raise ValueError(f"v207 incomplete video set: {method}")
        target_dir = comparison_root / "published" / method
        expected_targets = {f"{index:06d}-0.mp4" for index in range(PROMPT_COUNT)}
        stale = {path.name for path in target_dir.glob("*.mp4")} - expected_targets
        if stale:
            raise RuntimeError(f"refusing stale v207 VBench videos: {method}/{sorted(stale)}")
        ...
        if {path.name for path in target_dir.glob("*.mp4")} != expected_targets:
            raise RuntimeError(f"incomplete v207 VBench target set: {method}")
        methods.append(
            {
                "key": method,
                "role": row["role"],
                "runtime": "sf_native" if method == "sf_native" else "cache",
                "operator": row.get("operator"),
                "schedule": row.get("schedule"),
                "read_frame_equivalents": row.get("read_frame_equivalents"),
    ...
    payload = {
        ...
        "claim_boundary": (
            "Aggregate means are descriptive. Promotion requires paired full "
            "and late-half comparisons, an equal-budget Recent control, and "
            "automatic temporal safety. Dynamic Degree is audited but excluded "
            "from the promotion score."
        ),
    }
    ...
    if manifest_path.exists() and manifest_path.read_bytes() != encoded:
        raise RuntimeError("frozen v207 VBench manifest differs")
