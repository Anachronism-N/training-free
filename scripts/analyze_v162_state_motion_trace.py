#!/usr/bin/env python3
"""Audit whether v162 fresh-motion 4-pair permissive retrieval changed cache reads."""
...
def write_markdown(path: Path, report: dict) -> None:
    ...
    lines = [
        "# v162 Fresh-Motion 4-Pair Trace Audit",
        "",
        ...
        f"- Selected non-newest compatible pair: {aggregate['selected_not_newest_count']} (must be 0 for recency-only)",
        ...
        "",
        "This gate verifies that the 4-pair archive is active and recency-only readout always selects the newest eligible pair.",
        "",
    ]
...
def main() -> None:
    ...
    if [row["prompt_index"] for row in prompts] != list(range(PROMPT_COUNT)):
        raise ValueError("v162 trace prompt coverage mismatch")
    all_selected_ages = [
    ...
    mechanism_gate = bool(
        ...
        and aggregate["selected_not_newest_count"] == 0
        and aggregate["atomic_or_contract_failure_count"] == 0
    )
    report = {
        "version": 1,
        "experiment": "v162_freshmotion4_trace",
        "primary": PRIMARY,
        "mechanism_gate": mechanism_gate,
        "gate_definition": (
            "multiple eligible choices exist; recency-only readout selects "
            "the newest compatible pair; all reads remain atomic and within age"
        ),
        "aggregate": aggregate,
        "prompts": prompts,
        "claim_boundary": (
    ...
    if contract_failures:
        raise SystemExit(
            f"v162 trace contract failed with {contract_failures} violations"
        )
