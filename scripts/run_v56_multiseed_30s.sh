...
run(){ local gpu=$1 seed=$2 method=$3; shift 3; env CUDA_VISIBLE_DEVICES=$gpu PROMPTS="$PROMPTS" FRAMES=120 SEED=$seed STRENGTH=0 GATE_LAMBDA=0 RECENT_FRAMES=21 METHOD_TAG="${method}_s${seed}" RUN_ID="20260720_v56_${method}_s${seed}" "$@" bash "$ROOT/scripts/run_v35_pf_value_refresh.sh"; }
...
# Seed 1/2 paired PF and candidate.
run 1 1 pf CONFIG_PATH="$PF" STRUCTURED_MEMORY_ENABLE=0 &
run 2 1 ours "${COMMON[@]}" &
run 3 2 pf CONFIG_PATH="$PF" STRUCTURED_MEMORY_ENABLE=0 &
run 5 2 ours "${COMMON[@]}" &
