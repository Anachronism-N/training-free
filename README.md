...
The newest method candidate is **ProbeCache: Counterfactual Head Profiling and
Dual-Lifecycle Memory for Training-Free Long Video Generation**. It profiles
heads from controlled changes in native attention outputs, assigns persistent
and reactive roles, and gives the roles different active-memory lifecycles over
one shared clean archive.
Persistent heads use trust-conditioned distant Q/K recall; reactive heads use
recent novelty events. Both reuse Pyramid Forcing's post-prune RoPE and normal
attention path, and both fall back to PF when selection is uncertain. The
...
and improved temporal jump by 4.5%. Single-prompt 30s+ extrapolation remains
the primary task; prompt switching is secondary.
...
generation needs two different memory timescales: identity/layout heads need
reliable distant recall, while prompt/motion-sensitive heads need quickly
turning local events.

- **Profile:** measure prompt sensitivity and remote-history utility with paired
  native-output interventions.
- **Persistent lifecycle:** retain trust-conditioned archive writes and recall
  compatible distant frames.
- **Reactive lifecycle:** retain recent high-novelty event frames in the
  current prompt segment.
- **Direct realization:** insert selected frames into PF middle slots and
  preserve native fallback.

PF's labels and policies remain baselines/fallbacks and are not claimed as our
contribution. ProbeCache labels must be generated by the counterfactual profile
...
ProbeCache is integrated into the PF inference path:
...
All ProbeCache behavior is off by default. GPU inference has not been run on
this workstation.
...
The current machine has a CPU-only PyTorch environment but no CUDA runtime.
ProbeCache CPU mechanism tests and static compilation run here; actual PF
FlashAttention inference still requires the server `smoke` phase.
...
trust-conditioned writes, archive/top-k budgets, prompt priors, and abstention.
v78 remains the fallback if ProbeCache does not exceed PF.
